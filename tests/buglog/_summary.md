...
> 自动生成（`py tests/buglog/_gen_index.py`）·勿手改 · 最后更新：2026-07-29 16:33
...
| P1 高优先 | 4（B003/B006/B007/B011）|
| P2 中优先 | 3（B008/B009/B010）|
| 修复已提交 | 7 |
| 飞轮用例 | 27 |
| 最近飞轮报告 | report-2026-07-29-01-llm (56%·14/25) |
...
- **修复进度**：P0-4: harness.js 3 处 finalStep context 注入 newLayerCount 执行摘要（成功产出N层 / 未产出·禁编造） (31e2a00)
...
### B011 · 飞轮测试每例重复加载行政区 — 图层堆叠

- **严重度**：MED | **模块**：工具调用 | **复现**：1×
- **修复进度**：loadRange 加同名+同 srcName 去重检查·已加载则跳过 (adef900)
- **问句**：「（飞轮运行 25 例 llm 测试·每例调 loadRange('行政区')）」
- **关联**：[entry](open/B011-duplicate-range-layers.md) · CB-09
...
### B010 · 飞轮测试数据层命名混乱 — L2·e2e 不知所云

- **严重度**：LOW | **模块**：UI | **复现**：1×
- **修复进度**：e2e-seam.js loadPoints 组名 'L2 · e2e' → '测试数据 · 情绪点' (adef900)
- **问句**：「（飞轮测试运行时查看 Layers 面板）」
- **关联**：[entry](open/B010-e2e-layer-naming-confusion.md) · CB-09
...
- **最近报告**：report-2026-07-29-01-llm (56%·14/25)（`tests/reports/`）
