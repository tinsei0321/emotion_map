...
## R25 · 通道测试≠行为测试——模型编排链路验收必含真实复杂问题×3（PT-CB14·2026-08-24）

- **规则**：①凡经模型编排的链路（LLM 引擎/agent 循环），预检与验收必含**真实复杂分析问题×至少 3 次**看稳定性（概率性发散 1/3 量级只有多次才能暴露）；直调工具测试只证通道不证行为（"hi"能通≠分析能通）。②**检查代码自身要先验字段/接口**——自查工具的 bug 会生产"系统没数据"的假象（本轮 rag hits=0 误报：检查代码读 `hits` 字段而实际是 `results`，导致错误结论"RAG 无停车知识"入了审查报告）。
- **案例**：PT-CB14——主手预检只测 hi（通道绿）→用户实测四问题（行为红）；RAG 分歧双方各执一词→终局是检查代码字段名错误。
- **违反后果**：预检全绿给用户假信心；检查代码 bug 的错误结论会被下游引用扩散（审查报告已入库的错误条目需要专门更正）。
- **检查动作**：预检清单加"真实分析题×3"；检查代码写完先跑一个已知有结果的对照例。
...
- **追注（08-22·RAG 基线实录）**：评测器首轮 0%/100% 极端值必先查评测器自身（前缀归一化/匹配逻辑）再看被测系统；修正后逐条看 miss 明细——top1 正确而汇总未中=期望过窄（同源双源）非检索失败，防「假 miss」污染基线。
...
## R24 · 治理标注须引原文句子级语义；契约函数必须是唯一消费点（PT-CB9 泳道①②合流·2026-08-22）

- **规则**：①压旧/作废（superseded）类治理标注**必须引原文句子级语义**作依据——禁按文件主题/时间关系推断（PT-CB9 实录：把 03-10「新区增量·空间互斥·直接相加」读成「取代旧表」→ 38 chunk 误标 superseded → 索引重建后体检类 recall 瞬跌 37pp）；②**契约函数（如 load_chunks）必须是唯一消费点**——build_index 旁路直调 _load_* 拼接 → 治理/前注字段从未进 meta → 过滤与拼接全部惰性化（契约形同虚设）。
...
## R26 · 超时看门狗「有流量即续命」——预算检查必须在主循环内·禁只在静默超时分支（PT-CB15 SPIKE·2026-08-24）

- **规则**：给长驻流（SSE/JSONL/子进程 stdout）加总预算看门狗时·预算检查必须放在**主循环内**（每行/每帧处理后）——若只在「readline 超时」分支检查·只要对方持续 <心跳间隔 吐行（哪怕噪音/心跳通知）·总预算永不触发。「有流量=无限续命」·看门狗名不副实。
- **案例**：PT-CB15 codex_bridge.py:149-159——`asyncio.wait_for(readline, 15s)` 超时分支才查 `time.time()-t0 > 300s`·若 app-server 每 14s 吐一行无关通知·300s 预算失效。zcode 独立审计 Z-01 抓获（执行方自报与派发单均未提）。
- **违反后果**：用户挂死的请求永不超时·前端护栏（630s）成唯一防线·线程/连接资源长占。
- **检查动作**：写看门狗后问一句「对方持续小声说话会怎样」——写一个「高频行流」测试用例（mock 每 10s 吐行·断言预算仍触发）。
...
`）——换 SSE 库/代理改写换行即断·注释明示约定；
  ⑤**子进程 stderr 别 DEVNULL**——崩溃诊断唯一线索·环形缓冲末 4KB 随 error 事件携带；
  ⑥**配对 id 必须透传**：started/completed 事件的 item.id 透传·前端 begin/end 配对（本案 toolcall_id 空串致过程显示悬空）。
- **案例**：PT-CB15 Codex app-server 桥 spike（5925118）——六坑分别被 zcode/claude/kimi/Qoder 三方四方交叉发现（①三组全抓·⑤双组·③⑤⑥各一方·②执行方实测 K-4）。
- **违反后果**：换机即断（①）/静默丢行（②③）/换库即断（④）/排障瞎（⑤）/过程显示悬空（⑥）。
- **检查动作**：子进程桥 PR 自查六项清单；spike 也要最小测试（SSE 解析层可离线 mock 测·本案 5925118 零测试被 claude 抓）。
