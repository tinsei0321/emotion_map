...
2. **入库**：按该行「知识提交入口」落内容（md 笔记或知识库 dict·遵守该源纪律列）——**写入审批走 `docs/kb-pipeline-sop.md`**：三级分流（L1 派生资产自动通过 / L2 事实层写入全审·提案走 `docs/kb_inbox/` 草拟层带 verbatim+三分法+B2 自审三问 / L3 待裁队列）+ 更新频率分层（周=数据层禁入 RAG·月季=快审·年=全审+X-01）；
