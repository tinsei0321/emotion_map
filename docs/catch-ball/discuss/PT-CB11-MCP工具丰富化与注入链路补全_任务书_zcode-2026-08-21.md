...
## 七 拍板结果（用户 2026-08-21 深夜·已定）

1. **批次拆分**：✅ 接受两批（P1 先行 → P2 后批）。
2. **Kimi 首件**：**两件都派**——B-3 前端四件（B3-3~B3-6）+ area_stats（F_037·原 P2 件提前派给 Kimi）。派发 prompt 落盘 `PT-CB11-Kimi派发prompt_zcode-2026-08-21.md`。
3. **P1 三件工具执行手**：✅ Codex。函数级派发单落盘 `PT-CB11-P1三件工具派发单_Codex_zcode-2026-08-21.md`。

**并行结构裁定**（用户问「是否承接式」后明确）：非承接式·三线并行——①zcode：B3-1/B3-2 后端两件（在途）②Kimi：前端四件+area_stats（自测用内联 spec 不依赖后端修复）③Codex：三件工具（零逻辑依赖）。唯一协调点=mcp_server_emc.py 同文件分区改造（zcode 改 render_spec 区/Kimi+Codex 各加新函数+register 行·冲突面数行·后到者 rebase）。

---

> zcode 主手 · 2026-08-21 深夜 · PT-CB11 任务书·已拍板开工
