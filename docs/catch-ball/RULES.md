...
> **版本**：2.3 | **生效日期**：2026-07-28 | **更新**：2026-08-26（PT-CB18 W2-1·一事一档条款 §4.8）
> **变更记录（近 3 条）**：
> 1. 2026-08-26 v2.3：新增 §4.8 讨论文档「一事一档」（回应以小节追加同文件·收敛者就地改稿·文件头变更记录）——PT-CB18 W2-1，下一批起严格试用。
> 2. 2026-08-01 v2.2：CB-10·glm组加入（ZCode+GLM 5.2 以第三方评估者身份回归）。
> 3. 2026-07-28 v2.1：双阵营适用对象与 CB 统一入口定型。
> **适用对象**（双阵营）：
...
### 4.8 讨论文档「一事一档」（2026-08-26 · PT-CB18 W2-1 · 下一批起严格试用）

一个话题 = 一个文件，全生命周期同文件演进，禁多文件接力：

1. **回应 = 同文件追加小节**：回应方不新建回应文件，在原讨论文档内追加 `## 回应：{组名}-{日期}` 小节；
2. **收敛 = 就地改稿**：收敛者直接修订原文件（版本上抬如 v0.9→v1.0），不另出「收敛稿」新文件；
3. **文件头 3 行变更记录**：近 3 条变更（日期 + 版本 + 一句话），超出滚动丢弃；
4. 验收口径：下一批讨论产出文件数 = 1（模板/派发单等工具件不计）；历史已分体文档不回溯合并，只向前适用。
5. 例外：不同文体职责确需分离的（如任务书与其派发 prompt）允许分体，但必须在主文件头互指路径。
