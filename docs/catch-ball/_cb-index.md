...
| 项目 | 状态 |
|------|:---:|
| **全局核心路线** | **EMC×dsh 工程预期与双环境同步核心思想（08-21 用户定稿）**——测试期 dsh=外置实验驾驶舱，消费 EMC 数据/RAG/注册 Toolbox/render 契约并控制 8080 出图；稳定期 EMC 产品壳吸收测试链路。权威报告 `discuss/EMC-dsh工程预期与双环境同步核心思想_Codex-2026-08-21.md`（**v1.1 收敛定稿**·Qoder 评估 12agree/5partial/0disagree·五缺口 G1-G5 全吸收·环境配方化三步走 M0-M3·接口锁双写 PT-CB9·zcode 收敛报告 `EMC-dsh工程预期与双环境同步核心思想_zcode收敛定稿_2026-08-21.md`）；AGENTS v2.5 + KNOWLEDGE + context-map 已挂全局指针。各组涉及该线必须先读并遵守 |
