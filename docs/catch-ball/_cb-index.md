...
| 项目 | 状态 |
|------|:---:|
| **当前 CB 轮次** | **CB-21b**（城市更新专项规划资料集 **v2 升级**——G 盘真实资料三组协同全量读取 + 细颗粒度回填 EMC 知识库·待两组确认） |
| **上一轮** | CB-21（v1 反评价收敛·方案定稿）→ CB-20（全闭环·发版候选通过） |
| **当前环境** | **claude组（Claude Code + DeepSeek/GLM 5.2·开发主）** + **Codex** + **glm组（ZCode + GLM 5.2·评估）** |
| **当前分支** | `fix/emc-buglog`（与 origin 同步） |
| **最新讨论** | `discuss/CB21b-城市更新专项规划资料集v2_补充讨论_2026-08-09.md`（CB-21b·G 盘全量 900+ 文件·三组分工·GIS 已验证可读） |
| **最新进展** | **CB-17~20 全闭环**·**CB-21 v1 定稿**·**CB-21b v2 升级**：G 盘真实资料库（体检/GIS/政策/案例）三组协同全量读取 + 细颗粒度回填 + 双环境引用·pyogrio 已验证可读 GDB（22 图层） |
