...
### ④ 状态
`大南门数据专题 → 已实施闭环 + R7 截断修复` —— 大南门·二马路数据已接入 EMC 出口链路（/geo/catalog 可发现·/geo/zonal_stats 可聚合·/outlet_card 可出需求分析卡）·R7 截断已修复（阈值 1500·结构回切·按钮保留·文案场景化）。待：用户 F5 复验 R7 修复 + 浏览器 EMC 真实问答肉眼验证 + 时间轴 manifest 后置。
