...
| 启动命令 | `python -m streamlit run apps/app_main.py` |
...
- CLI / Tkinter / Streamlit **三个入口调用同一个** `run_analysis_task()`
...
| 序号 | 字段名 | 类型 | 说明 |
|------|--------|------|------|
| 14 | `lon` | float | **WGS84 经度 (EPSG:4326)** — GeoJSON 导出基准 |
| 15 | `lat` | float | **WGS84 纬度 (EPSG:4326)** |
| 16 | `x_cgcs2000` | float | CGCS2000 EPSG:4546 投影 X（米） |
| 17 | `y_cgcs2000` | float | CGCS2000 EPSG:4546 投影 Y（米） |
| 18 | `id_e` | str | 稳定行标识符（格式：`e0001`~`eNNNN`） |
| 19 | `scope` | str | 空间过滤使用的边界名称 |
| 20 | `in_scope` | bool | 是否通过范围过滤 |

#### L2 新增字段 (5 列, 共 25 列)

| 序号 | 字段名 | 类型 | 说明 |
|------|--------|------|------|
| 21 | `score` | float | SnowNLP 综合情绪得分 0~1 |
| 22 | `polarity` | str | 五级极性 |
| 23 | `keywords` | str | jieba 情绪关键词（逗号分隔） |
| 24 | `l2_confidence` | float | L2 置信度 0~1（文本长度归一化） |
...
| 指标 | 预算 | 测量点 |
|------|------|--------|
| 地图首次渲染 | ≤ 3s | `create_base_map()` + `streamlit_folium` |
...
| 包 | 版本要求 | 用途 |
|----|----------|------|
| streamlit | >=1.35 | 前端应用框架 |
| streamlit-folium | >=0.20 | Streamlit Folium 集成 |
| folium | >=0.17 | 地图可视化 |
| pandas | >=2.0 | 数据处理 |
