...
| **选项** | 一行多条、需单选/多选（如分析类型卡） | 选中态=**蓝底白字**（工具条设计语言） |
| **胶囊** | pill 圆形组件（如类型/表现喜怒哀乐愁急盼） | 选中态=**颜色填充**（不用蓝底白字） |
...
### 4.7b 色板自动 + 双语义色板
- 色板不再由用户手选——随"分析类型 + 数据层级 + 特性"自动选定（`computeStyle`）；③区降级为只读色板预览。
- **双语义色板并存，不可混**：
  - 总体情况（地形/网格）= 红蓝绿高程语义（terrain-9 / 绿3/红3/蓝3）：山顶积极绿、山腰中性蓝、山底消极红。
  - 类型细分（积极/消极/中性）= 胶囊色渐变 7 段（分类语义）。
- 7 大类胶囊色 ↔ classify-7 色板一一对应、同源同序（改色全局同步：胶囊/色板/Overview/图例）。

### 4.7c 分析类型三层递进
- ①分析类型分 3 排 3 层级，粒度递进不混淆：
  - **总体情况**（地形/网格）：综合密度/强度，看舆情热度与正负高低洼地，**不涉及类型/表现**。
  - **类型细分**（积极/消极/中性）：进一步看"哪种情绪"，用到类型/表现胶囊。
  - **多维归因**（情绪因子/要素归因）：归因，暂占位。
- 层级决定②类型/表现胶囊是否启用（总体/归因禁用）、特性下拉逻辑、色板语义。

### 4.7d L2 分数 ↔ 类型：两个独立维度（数据语义核心）

L2 数据有两个**互相独立**的情绪维度，不可混淆（影响多个分析的取色与解读）：

- **L2 分数**（SnowNLP，0=极端负面 → 1=极端正面）→ 五级 polarity（[emotion_analysis_v1.py:194](SCRIPT/emotion_analysis_v1.py#L194)：≤0.2 非常消极 … >0.8 非常积极）。语义=**正负倾向强度**。
- **情绪类型**（primary_emotion，喜怒哀乐愁急盼细分）= **词典独立分类**（`classify_emotion_type`）。语义=**具体类别**。
- 两者**不完全挂钩**：一条数据可能分数偏积极，但类型是"期待建议"（中性）；分数只定正负浓淡，类型定具体是什么。
- **L2 点 polarity 色**方向正确（Very Positive 鲜绿 / Very Negative 深红，[state.js](frontend/js/state.js) L2_POSITIVE/L2_NEGATIVE），无需改。
- 历史 bug 在**类型细分色板**（gradientStops 端点顺序反，高值色落在低密度端），非 polarity 点色。修法：端点反转，高值色归位高密度（见 `de2bd54`）。

> 排错心法：L2 颜色"不对"时，先分清是 **polarity 点色**（按分数/极性，已对）还是**类型细分色板**（按密度映射大类色，易端点反），两者修法不同。
...
3. **三组件设计 token**：
   - **栏**（占满整行）`.is-bar-sel` = 浅蓝填充、无边框（Layers 行、③色板选择）
   - **选项**（一行多个）`.is-opt-sel` = **蓝底白字**（参考工具条；分析类型卡等）
   - **胶囊**（pill）= 颜色填充（类型/表现喜怒哀乐愁急盼）
   - 未选中：白/透明；悬停：浅灰（栏与选项一致）
...
9. **联动排除（全局）**：各选择栏互相联动，**无对应字段的层级/选项自动排除**（如类型细分积极/消极/中性是 L2 专属，选中即锁 L2；L1 无情绪字段则类型/表现胶囊禁用）。
10. **独占显示（底层逻辑，勿擅改）**：生成新热力图时隐藏其他所有图层；须保留 `dispatch layers:changed` 保侧栏眼睛状态同步（否则眼睛表象"无效"）。
...
|------|--------|----------------|------|
| 06-20 | `581d608` | ①排版太乱 → 三组浅灰圆角背景+4px间隙+等宽，标题最深字；按钮白底圆角阴影无线框、悬停灰选中蓝白字、间隔2px；栏/选项/胶囊三组件设计 token；弹窗460px免滑动；③还原"显示样式"+色板选择（栏语义）；分割线加粗；文案正式化 | `dialog.css` `index.html` `heatmap-tool.js` |
| 06-20 | `cd015c4` | L1 表达优化 → 新增"热度值=情绪强度×置信度"属性，3段动态分位色板（浅橙→深橙红）；L1 点按 hotness 渲染（原用不存在的 score 全中色）；胶囊/popup/图例改热度值；置信度移情绪强度下+坐标弱化+置信度/热度值 i 解释 | `state.js` `map.js` `popup.js` `settings.js` `sidebar.js` `popup.css` `index.html` |
| 06-20 | `f93719a` | 生成热力图关闭其他图层失效（阶段0误删）→ 恢复独占显示（保留 layers:changed 保眼睛同步）；选中性出现 L1 → constrainLevelOptions 锁 segment tier 到 L2；L1"可分析"标注莫名 → 去掉 | `heatmap-tool.js` `panel.js` |
| 06-20 | `de2bd54` | 积极/消极/中性热力图颜色反转（喜绿应在高密度却写低端）→ gradientStops 端点反转，高值色归位；去 ramp 名口语（喜→乐） | `state.js` |
| 06-20 | ⏳ 待办 | 高级参数设置有 bug（用户暂不修，记录待查） | — |
| 06-20 | `fa9e31c` | 分析类型重分层 3 排（总体/细分/归因，去情绪归类）；色板随类型自动（③降级只读预览）；特性下拉矩阵；②胶囊按层级显隐；生成按钮拆 2D/3D；右下热力图图例；表达（去"选择"、极性→特性）。bug3 由 terrain+L2 锁综合消解 | `heatmap-tool.js` `index.html` `dialog.css` `legend.css` `heatmap-legend.js` `main.js` |
| 06-20 | `5511d09` | 7 大类胶囊 ↔ classify-7 色板一一对应（喜绿/乐橙/怒红/哀紫红/愁紫/急深蓝/盼天蓝）；积极/消极/中性格色板由胶囊色渐变 7 段；新增 terrain-9/绿3/红3/蓝3（总体红蓝绿高程语义，与分类色板区分）；gradientStops 工具 | `state.js` |
| 06-20 | `065e8f6` | 5 bug：popup 折叠改判可交互要素（排除热力图像素）；范围 popup 去 accent border（f9da7c1 引入）；layerLevel 从 paint._ui.level 推断（L1 彩虹不再误标 L2）；generateHeatmap 不再独占隐藏其他层 | `popup.js` `popup.css` `state.js` `heatmap-tool.js` |
