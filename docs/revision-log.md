...
> 开发时主看本文件即可（历史修订见第 5 节）。本节按**功能模块**组织——每模块一个主干，模块下挂任务，反映项目模块化推进思路。
> 状态：⬜待办 / 🔄进行中 / ✅完成 / ⏸阻塞 / ❌否决。新分支产生即追加（AI 全程维护，不靠会话记忆）。
...
├─ Toolbox 工具箱 [🔄]
│  └─ 多维归因分析 ⬜（自 KDE 弹窗①剥离）
├─ 图层/设置/Overview [🔄] ── 联动修复 ✅ / Layers 分组重做（核心四件）✅ 06-21
├─ 数据管道 L0→L4 [🔄] ── L1→L2 验证
├─ Import 管道 [✅]
├─ 外壳/控件/视觉 [✅]
├─ 数据采集 Scrapy [✅]
...
- **核密度分析（KDE）弹窗** [🔄]：
  - 批1 快赢 [🔄]：1a 预览图换 kepler 截图 ⏸（暂搁置，等 terrain/factor 截图补齐）；1b 小类配色按大类派生 ⬜
  - 批2 全局时间轴 [⬜]：2.0 数据前置（时间演变数据）；2.1 `time-axis.js`；2.2 filter 动态化（架构转折）
  - 批3 3D 渲染 [⬜]：地形凸凹 + 网格柱体（deck.gl）
  - 批4 时间对比 [⬜]：A/B 双窗（依赖批2）
  - 批5 图层分组 [🔄]：5A 自动归类 ✅（06-21，Layers 面板渲染层聚合，5 组自动归类）；5B 自由编组 ⬜
  - 结构清理 [✅ 06-21]：多维归因卡片组（factor/attribution）剥离至 Toolbox，①回归 2 排（总体情况 / 类型细分）
  - 待决策：批2粒度；批3地形 vs 柱体（批1预览图 ⏸ 已搁置）
