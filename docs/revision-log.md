...
7. **任务树全程维护**：第 7 节"任务路线图"是树状任务跟踪（主线+子分支+临时分支）。产生新分支（修 bug 发现新任务 / 某批分前置 / 新需求）→ 立即追加到对应节点 + 更新状态。不靠会话记忆。

---

## 7. 任务路线图（树状任务跟踪）

> 一站式开发视图：第 5 节 = 历史修订，本节 = 未来计划 + 临时分支。开发时主看本文件即可。
> 状态：⬜待办 / 🔄进行中 / ✅完成 / ⏸阻塞 / ❌否决。新分支产生即追加（AI 全程维护，不靠会话记忆，会话分段/新会话读此即接上）。

### 主线（批1–5）

- **批1 快赢** [⬜]（小，无依赖）
  - 1a infopanel 预览图换 kepler 官方截图（`assets/analysis-previews/` 已备 4 png 未引用）
  - 1b 表现小类配色按所属大类色系派生（`EMOTION_MACRO_MAP`）
- **批2 全局时间轴** [⬜]（中，核心，依赖 `publish_time` ✅）
  - 2.0 数据前置：重生成"有时间演变"模拟数据（整治前后 / 季节 / 周期故事）
  - 2.1 `time-axis.js` + css（双滑块范围 + 播放 + 粒度切换）
  - 2.2 架构转折：热力图从"生成时 `filterFc` 切片"→"全量 source + MapLibre `filter` 表达式动态筛选（`publish_time`）"；时间窗存 `paint._ui.timeWindow`（H 继承）
- **批3 3D 渲染** [⬜]（大，依赖 deck.gl）：地形凸凹（`HexagonLayer`+elevation，`diverging-rg`）+ 网格柱体（`GridLayer`，`grid-warm`）。承接本次 3D 占位。
- **批4 时间前后对比** [⬜]（大，依赖批2）：A/B 双时间窗分屏 / 叠加（过去红 / 现在绿）
- **批5 图层分组** [⬜]（中，独立）
  - 5A 自动归类（`generateHeatmap` 按 level+特性归组，复用 `parentId` / `toggleGroupEye`）
  - 5B PS 式自由编组（多选 + 拖拽入组 + 嵌套）

**待决策**（各批启动定）：批1 预览图取舍；批2 时间粒度默认；批3 地形 vs 柱体优先。
**依赖链**：批2 是架构转折（filter 动态化）→ 批3 / 批4 依赖；批5 独立可先行。

### 临时分支（修 bug / 打磨途中产生）

- [✅] L1 热度值体系（强度×置信度，3 段动态分位）— 源：修 L1 不显示
- [✅] 类型细分色板方向反转（gradientStops 端点）— 源：积极热力图颜色反
- [✅] 同步联动（Overview / popup / 图例 `layers:changed`）— 源：Overview 显错层
- [✅] ①三组排版（纵向，每行一个）— 源：排版乱
- [✅] serve.py 自动 `?v` 注入 + 启动清端口 — 源：浏览器缓存残留
- [⏸] 高级参数 bug（C5，暂不修）
- [⬜] …（后续增生即追加）
