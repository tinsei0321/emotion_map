...
> 📍 **最新动态（07月26日）** · 本节按板块分组、组内倒序；最新工作 = **5.219 EMC chip tip 标签对齐修 + 优化↔diagnose 衔接确认 + 自成长 P2 评估**（本次，分支 `main`）。上一轮 5.218。最近：
>
> - **5.219 EMC chip tip 标签对齐修 + 优化↔diagnose 衔接确认 + 自成长 P2 评估**（用户反馈 chip tip 没实现 + 衔接疑问 + 自成长疑问·主线程·未派 subagent）：① **chip tip bug**——[_liveRecTip](frontend/js/ai_qa/panel.js#L1004) 匹配 `['密度','排序'...]` ≠ [_liveRecognize](frontend/js/ai_qa/panel.js#L972) 实际 push `['热力图','Top N 排序','4×5 归因'...]` → 找不到 → tip 空 → 第二行不显示。**修**：_liveRecTip 改 `tags.includes` 匹配实际标签（热力图/Top N 排序/4×5 归因/区域对比/Gi* 热点/用地占比/缓冲区/范围裁取·8 类）·input 事件已接通（:1942）·修后实时显示 tip。② **优化↔diagnose 衔接确认**（用户逻辑·两条路径）：路径 1（点优化键→Flash 精准化→输入框→send→diagnose·5.218 已实现）+ 路径 2（直接 send→diagnose 内部 prompt 工程 = 隐式优化+计划·早已工作）——两条路径都通 diagnose·衔接已实现。③ **自成长评估**（用户问"会越来越聪明吗"）：**运行时不自动学**（episode 记不学·_tplHitRate 只统计 gate）；**开发层学**（手动调 prompt/CB/memory·基于反馈·5.200-5.218 迭代证明）；**自动学是 P2**（episodes golden+回归 gate / RAG 注入 / 反馈驱动·需大量用户使用记录·开发阶段不足）——用户定 A（先修 bug·自成长 P2）。commit b5e0675·**待用户 push**。
>
> - **5.218 EMC prompt 优化方向修正（精准化扩充+用户语气·推翻简洁条目化）+ chip 两行 tip + 大幅减 token**（用户精确反馈·前几轮理解错·主线程·未派 subagent）：**用户校准**（关键·推翻前几轮）：① prompt 优化 = **精准化扩充 + 逻辑梳理**（非改写/条目化/启发）——模糊词→具体层名 + 极性细化·逻辑不清才梳理；② **用户提问语气**（"分析 X 范围内 Y 的 Z 分布"·非指导用户"你应该/建议"）；③ **启发是 chip/CPD 职能**（非 prompt 优化）·prompt 不启发（不加用户没提的维度）；④ chip **两行**（第一行短语·第二行方法 tip 长句"tip：通过 density 分析 X 的极性分布"）；⑤ **大幅减 token**（等待太久）。**改**：① [OPTIMIZE_TEMPLATE](ai_qa/prompts.py) 重写（精准化扩充 + 用户语气 + few-shot 用户理想例「分析「XX 范围」范围内的「YY 情绪点」的整体极性空间分布」+ 推翻简洁条目化 + 精简 ~150 token vs 5.217 ~400）；② [buildOptimizeContext](frontend/js/ai_qa/tools.js) 极简（只层名+kind·~200 token vs ~500）→ 总 system+context ~350 token（vs 5.217 ~900）·Flash <2s；③ chip 两行 [_renderRecognize](frontend/js/ai_qa/panel.js)(tags,tip) + [_liveRecTip](frontend/js/ai_qa/panel.js)（方法长句·density/zonal/rank/compare/hotspot/area·基于意图+区名）；④ css .aiq-rec-line/.aiq-rec-tip（column 两行）。**红线**：不动 diagnose/schema/orchestrate。**验证**：语法过；浏览器（输入「上传范围和情绪点分析分布」→优化精准化层名+极性细化·非 JSON/条目化·<2s·用户语气；chip 两行短语+tip）留用户。commit bdf4d3a·**待用户 push**。
>
> - **5.217 EMC prompt 优化工程系统化（bug JSON 根治 + 简洁条目化风格 + 超时 <3s + Layers 数据消费 + 智能场景）**（用户实测 bug（优化出 JSON 代码块）+ 风格反馈（冗长→简洁条目化）+ feature 需求·plan 系统设计·主线程·未派 subagent）：**用户反馈**（关键）：① bug——优化后显 `{thought,action:ask_user}` JSON（diagnose/agent 格式·非优化文本）·根因 [OPTIMIZE_TEMPLATE](ai_qa/prompts.py) 弱约束·Flash EMC JSON 训练惯性；② 风格——当前优化冗长段落·要**简洁 + 条目化（分号短句）+ 去修饰 + 关键词驱动 + 高效凝练**（用户给理想例）；③ 超时 5-10s；④ feature——消费 Layers 数据 + 智能场景。**step1 bug+风格根治**（2bcc08c）：[OPTIMIZE_TEMPLATE](ai_qa/prompts.py) 强化（首末强约束禁 JSON/thought/action/代码块/冗长段落 + few-shot 2 例·用户理想例教简洁条目化 + 数据消费关联层名 + 不增维度）+ [_parseOptimize](frontend/js/ai_qa/panel.js) 拦 JSON（`{thought/action}` → 空）+ _toggleOptimize onOptimize 流式拦 JSON（不写输入框防代码块闪）+ 失败 placeholder 提示。**step2 超时**（65add16+b491c23）：[buildOptimizeContext](frontend/js/ai_qa/tools.js)（精简 grounding·层名+kind+字段role·同步·非样本值·~500 token vs buildContext 2-4k）+ _toggleOptimize 用它（替换 await buildContext·同步·快）。**feature 1/2**（含 step1 OPTIMIZE_TEMPLATE）：数据消费（"上传的数据"→关联具体层名）+ 智能场景（范围+点→分布梳理·守不增维度·缺要素提示）。**客观评估**：大工程·meta-prompt 多轮迭代（3-5 轮·用户反馈驱动）·workbuddy 级非一次成型。**红线守约**：optimize phase 独立·不动 diagnose/schema/orchestrate。**验证**：node --check + py_compile 语法过；浏览器（输入「中心城区情绪分布」→优化出简洁条目化 prompt·非 JSON/代码块·<3s·不增维度）留用户。commits 2bcc08c/65add16/b491c23·**待用户 push**。
>
> - **5.216 EMC 优化键四修（bug 缺数据代码块 + 位置 + 撤销 icon + chip 发散启发）**（用户实测反馈 4 点·主线程·未派 subagent）：① **bug 缺数据显代码块**——[renderAnswer](frontend/js/ai_qa/panel.js#L446) marked.parse 把反引号当代码·[composeGapCard](frontend/js/ai_qa/harness.js#L167) 动态值（needed/gap/failedObs·Flash data_plan 字段）可能含反引号·[_esc](frontend/js/ai_qa/harness.js#L160) 不转反引号 → marked 代码块。**修**：_esc 加反引号转义（` → &#96;·防动态值致代码块）。② **优化键位置**——[.aiq-optimize](frontend/css/ai_qa.css) margin-left:auto 推到发送键左 + margin-right:2px 间隔（用户要"右移·与发送键 2px"）。③ **撤销 icon 重设计**——旧回箭头丑·换 Lucide undo（左上箭头 + 弧 `M9 14 4 9l5-5` + `M4 9h11a5 5 0 0 1 0 10h-5`·清晰）。④ **chip 发散启发**——[_liveRecognize](frontend/js/ai_qa/panel.js#L960) 升级·基于输入类型发散更多要素（区名→子地点二马路/CBD/东站；密度→热力图/核密度/热点分布专业名词；多地点→区域对比；归因→4×5；排序→Top N；最多 6 chip·flex-wrap）·举一反三启发用户。**注**：chip 发散（启发更多要素）vs 优化键不增维度（梳理已有）——两者语境不同（chip 输入提示发散·优化尊重原意不增）·并存合理。commit fd077e0·**待用户 push**。
>
> - **5.215 EMC LLM prompt 优化键（业界级·Flash 流式 <3s·不增维度 + 恢复输入提示 chip）**（用户反馈 5.214 代码改写差·要业界 workbuddy 级 + 不增维度 + 保留输入提示·主线程·未派 subagent）：**用户校准**（关键）：① prompt 工程 = 把用户 NL **逻辑/用词/要素/目的梳理**（非大面积改写）·让用户对需求更清晰；② 业界都是 LLM（workbuddy/GPT 等·meta-prompt 拆条目·1-3s）·纯代码做不到（5.214 验证）；③ **不增加维度**（核心约束）——只梳理用户已有要素·绝不引入用户没提的（没提时间不加 T1/T3·没提归因不加 4×5·没提对比不加对比）；④ 启发 = 需求更清晰（非教用户问更多）；⑤ 保留输入提示 chip（5.213·对话框下）。**改**（LLM 优化 + 恢复 chip）：① [build_optimize_prompt](ai_qa/prompts.py)（meta-prompt·Flash 流式·**不增维度**·只梳理已有要素 + 具体化 + 逻辑化 + 操作化·自然语言非条目编号·无 MANIFESTO 精简提速）；② [router](ai_qa/router.py) phase 'optimize' + [stages.optimizeStep](frontend/js/ai_qa/stages.js)（streamChat onOptimize 流式写入输入框）；③ [_toggleOptimize](frontend/js/ai_qa/panel.js) 改 LLM async·三态 sparkle⇄loading（spinner + 输入框 placeholder「prompt 优化中…」双提示）⇄undo（撤销 _originalInput）；④ **恢复 5.213 输入提示 chip**（[_renderRecognize](frontend/js/ai_qa/panel.js)+_liveRecognize·对话框下·代码几 ms·预览非 LLM·与优化键并存）；⑤ 删 5.214 代码版 _optimizePrompt/_OPT_TEMPLATES。**<3s 保证**：Flash 快档 + meta-prompt 精简 + 流式首字 <1s（打字感）。**红线守约**：新 phase+prompt 独立·不动 diagnose（DIAGNOSE_TEMPLATE eval-anchor 保）/ schema / orchestrate 主循环。**验证**：node --check .mjs + py_compile 语法过；浏览器（输入模糊→点优化→1-3s 流式出"梳理已有要素"优化 prompt·非条目·不增维度→启发感→撤销 + 对话框下 chip 实时提示）留用户。commit 30237cb·**待用户 push**。
>
> - **5.214 EMC 一键优化 prompt 键（反转 5.213 显示→真功能键·代码改写 + 撤销 + sparkle/undo icon）**（用户澄清"要功能键非显示"·主线程·未派 subagent）：**用户澄清** = 5.213 的"显示已识别 chip"理解错·要**真功能键**：点击→代码改写优化 prompt 入输入框·变撤销 icon·再点恢复原文·毫秒级无 LLM·发送键左同尺寸。**反转 5.213**（删 chip 显示：#aiq-recognize DOM + _renderRecognize + input 事件扩展 + send 清 + css .aiq-recognize/.aiq-rec-*）+ **保留 _liveRecognize/_REC_***（优化复用）。**加优化键**：① [#aiq-optimize](frontend/index.html#L238) 按钮（发送键左·32×32 同 [chat-send](frontend/css/ai_qa.css#L185)·sparkle icon）；② [_optimizePrompt](frontend/js/ai_qa/panel.js#L984)（代码·几 ms·非 LLM）= 指代展开（这边→getSelectedLayer 名）+ 口语规范（看看→分析/怎么样→情绪状况）+ 意图模板补全（[_OPT_TEMPLATES](frontend/js/ai_qa/panel.js#L978) 归因/排序/密度/对比/缓冲/热点/面积/裁取 → 专业 prompt·{D}=区名 {S}=尺度）；③ [_toggleOptimize](frontend/js/ai_qa/panel.js#L1004) 优化/撤销切换（.is-optimized class·css 切 sparkle⇄undo svg）+ _originalInput 存原文；④ send 后 _resetOptimize 重置。**红线守约**：纯前端（index.html + panel.js + ai_qa.css）·不动 diagnose/schema/orchestrate。**纯代码改写**（用户明确毫秒级无 LLM）·非 LLM 深度优化（接受改写有限·深度语义需 diagnose LLM）。**双主题**（Light token + [data-theme=dark] EMC 覆盖·is-optimized 品牌色 #D97757 强调）。**验证**：node --check .mjs 语法过；浏览器肉眼（输入"这边怎么样"+选中西陵区→点优化→"分析西陵区的情绪状况..."→点撤销恢复）留用户。commit d6e9add·**待用户 push**。
>
> - **5.213 EMC UI 显化已识别标签（输入时实时 sparkle + chip·prompt 工程透明化·纯前端非 LLM）**（用户想法"发送键左侧加 prompt 优化键"·校准后选 A UI 显化·主线程·未派 subagent）：**用户原想** = 发送键左侧加 LLM 优化键（点击优化 NL）。**校准**（否决 LLM 优化键）：① "优化 NL"需 LLM（Flash 2-5s）·与 diagnose 重复（两次 LLM·4-10s）；② 改写风险（误规范化）；③ "后台自动跑"= 现状（diagnose + resolveCoref 5.212）。**真值方案**（用户选 A·UI 显化已有 prompt 工程）：输入时实时显示「[sparkle] 西陵区 · 归因 · 中观」chip 行——[_liveRecognize](frontend/js/ai_qa/panel.js#L947)（panel.js·代码关键词·几 ms·非 LLM）→ tags[]（区名/排序/对比/密度/缓冲/归因/热点/面积/裁取 + 宏/中/微）→ [#aiq-recognize](frontend/index.html#L226) 行渲染（textarea 下·不挤 emc-input-left）。**sparkle 圆角正方图标** 32×32（同 [chat-send](frontend/css/ai_qa.css#L185) 尺寸·border-radius 6·AI 已识别标识·Light design token + [data-theme=dark] EMC 覆盖·双主题统一）+ chip（区名/意图/尺度·填充式胶囊·设计语言一致）。**定位**：预览（代码大概·输入时自查）·diagnose（发送后 Flash 精确卡）·不重复。**红线守约**：纯前端（index.html + panel.js + ai_qa.css）·不动 diagnose prompt/schema/orchestrate。**验证**：node --check .mjs 语法过；浏览器肉眼（输入"西陵区情绪归因"→显 sparkle·西陵区·归因·中观 chip·空隐藏·send 清）留用户。commit 0a02412·**待用户 push**。
>
> - **5.212 EMC grounding 指代标注（NL 预处理·resolveCoref·让 diagnose 不靠猜指代）**（用户认知校准后选 A·"EMC NL→prompt 工程是否内嵌"讨论·主线程·未派 subagent）：**用户原问** = EMC 是否内嵌 prompt 工程（NL→定制 prompt）·之前是否忽略。**校准**（讲清 EMC NL→计划 完整链路）：① EMC 不是两层（NL→prompt / prompt→计划）·是**一步**（diagnose Flash LLM·NL→计划卡·2-5s·计划阶段唯一 LLM）；② **prompt 工程已重度内嵌** [build_diagnose_prompt](ai_qa/prompts.py#L221)（MANIFESTO + DIAGNOSE_TEMPLATE + 8 附录 + few-shot 6 例 + grounding·就是"NL 喂进定制 prompt"）；③ "NL→prompt 不需 LLM·几秒内"——纯代码几 ms（buildContext/_quickIntent/派生）已几 ms·diagnose Flash 是瓶颈（NL 理解需 LLM·不可代码化）；加独立 NL→prompt 层**价值边际**（LLM 已理解 NL·冗余 + 误规范化风险）。**真值优化**（用户选 A）：[resolveCoref](frontend/js/ai_qa/tools.js#L233)（tools.js·几 ms·非 LLM）检测问句指代词（这边/这个区/这里/刚才/上次）→ 标注当前聚焦对象（activeAnalysis/selectedLayer）+ 上轮结果（priorTurn.done）→ grounding 显式关联 → [orchestrate diagnose 前注入](frontend/js/ai_qa/harness.js#L449) ctx.context。让 Flash 多层加载时知"这边"=哪个·不靠猜。**保守**：对象不存在不标（不误导）；不改写 NL（只标注关联·防误规范化）。**红线守约**：ctx.context 增强（类似 :461 formatRegistry 注入范式）·不动 diagnose prompt/schema/DIAGNOSE_TEMPLATE/主循环。map 焦点空间查询（点 in polygon→区名）标 YAGNI（复杂·后续）。**验证**：node --check .mjs 语法过；飞轮（选中西陵区+问"这边情绪"→diagnose 知指代）留用户。commit e3c4266·**待用户 push**。
>
> - **5.211 EMC Layer Manifest 最小版（backlog ④ 收尾·fieldCard Promise 缓存 + layers:changed 预计算）**（用户"继续 EMC 修复工程·不验证·全部跑完后再验证"·backlog ④ 现状盘点后做·主线程·未派 subagent）：**backlog ④ 现状**——原痛点「字段识别分散、无统一 manifest」（K3 §2.2/§3.2）**已被 5.200-5.205 逐步加的 [getFieldCard:99](frontend/js/ai_qa/tools.js#L99) 缓存解决**（profile→resolveRole→LLM 兜底→`_fieldCardCache` by layerId + [_fieldSamples:415](frontend/js/ai_qa/tools.js#L415) 复用缓存 + DataEye 消费·buildContext 不重算）。**剩两小缺口**：① **首次慢**（getFieldCard 首次 buildContext 触发·miss 字段 LLM 兜底阻塞首次 diagnose 首字）；② **并发风险**（`_fieldCardCache` 存值非 Promise·并发首次重复 LLM）。**改**（[tools.js](frontend/js/ai_qa/tools.js) 单文件）：① **getFieldCard Promise 缓存**（`_fieldCardCache` 存 Promise·并发首次命中同一 Promise·不重复 LLM·调用方 await 不变）；② **layers:changed 监听预计算**（fire-and-forget·新可见层导入即算 fieldCard·首次 diagnose 命中缓存·治首字延迟·C9 根治·E2 治感知）。**红线守约**：tools.js 单文件·不动 import.js（addLayer 通用·依赖单向）/ diagnose prompt / schema / orchestrate。**后续 YAGNI**（值边际·不做）：结构化 manifest（`l.manifest={srcId,kind,bbox,fields}` 重组）+ 消费统一（buildContext/diagnose/SKILL_DEFS 读 l.manifest）——现有 getFieldCard+DataEye 已覆盖字段认知核心·待痛点暴露再做。**验证**：node --check .mjs 语法过；运行时（首次 diagnose 首字更快 + 并发不重复 LLM）留用户飞轮。commit 80d09c4·**待用户 push**。
>
> - **5.210 EMC 专题 E1 · 多步链 runChainPath（治 C3 多步超时·0 中间 LLM 轮·CHAIN_REGISTRY + runChainPath + orchestrate 分流）**（用户"推进 E1（进 plan）"·合并 toolbox→main 后首项·主线程·未派 subagent·3 step 独立 commit）：**C3 痛点** = 复合问（「西陵区的商业用地」「范围密度」）diagnose 选 template=multi → 走 [orchestrate while-loop](frontend/js/ai_qa/harness.js#L548) ReAct·每轮 LLM·多步链 p^N 概率超时。**3 step**：① **E1-step1 CHAIN_REGISTRY**（[stages.js](frontend/js/ai_qa/stages.js) 纯前端·extract_overlay / clip_density 标准链·chain_id 由 harness _deriveChainId 派生**非 Flash 选·不进 diagnose prompt·免后端双端镜像**；[eval](tests/eval_template_flash.py) +2 multi 用例冻结 31/37=84% ≥80% gate·「西陵区的商业用地」→multi 命中 chain 覆盖·f5b4078）。② **E1-step2 runChainPath**（[harness.js](frontend/js/ai_qa/harness.js) 新增·类比 runTemplatePath 循环 chain.steps·**复用 $n/_stepResults**（[tools.js ref:153](frontend/js/ai_qa/tools.js#L153) 工具内自动解析·不重造 layerId 管理）+ setToolContext + runTemplatePath 出口范式（链中断/零图层→ask_user·成功→finalStep+_verifyClaims）；_resolveChainParams 填 {question}→区名/{boundary}→diagnose.params·$n 透传·3f31c15）。③ **E1-step3 orchestrate :537 分流**（[:534](frontend/js/ai_qa/harness.js#L534) single 分流后加 `template==='multi'` + _deriveChainId 命中→runChainPath·未命中落 while-loop·**最小 if 增量**不动 single/while-loop/出口裁定；_deriveChainId 问句匹配 CHAIN_REGISTRY triggers 首个命中·3dbfcdf）。**红线守约**：前端为主（stages + harness）·不改后端 paradigm/prompts/schema/diagnose prompt；orchestrate 最小 if（multi 分支）·非主循环/分流/出口裁定条件改；$n 复用既有 _stepResults（不重造）。**验证**：node --check .mjs 三步语法过；chain 执行（飞轮「西陵区的商业用地」→extract_overlay 0 LLM 中间轮 + 「范围密度」→clip_density）留用户。**风险**：覆盖面窄（仅标准链·非标准 multi 仍 while-loop）+ triggers 误匹配（保守起步 2 链·飞轮验后扩）。commits f5b4078/3f31c15/3dbfcdf·**待用户 push**。
>
> - **5.209 EMC 专题 E3 · partial 出口消费 _renderState（治假完成制度化·渲染失败层不再计"实际产出"）**（用户"继续下一步"·E2 后推 E3·主线程·未派 subagent）：**假完成场景** = 工具报成功（newLayerCount++）+ 图层入列表（名在）但地图未真渲染（_renderState=failed/partial）→ 出口对账 [:690](frontend/js/ai_qa/harness.js#L690) `_actualNames` + [:225](frontend/js/ai_qa/harness.js#L225) `_verifyClaims` actual 只看 `l.name` → 声称的若名在=不 missing → **判 EXIT_RESULT（假完成）**。**改**（[harness.js](frontend/js/ai_qa/harness.js)·两处对账真值源）：`actual/_actualNames` 加 `(l._renderState || 'ok') === 'ok'` 过滤 → 渲染失败层不计"实际产出" → 声称的若渲染失败=missing → EXIT_PARTIAL 标注 + _verifyClaims 拦截。**两处口径一致**（runTemplatePath/partial-gate 经 _verifyClaims + orchestrate while-loop 对账）。_renderState 信号来自 [map.js:461](frontend/js/map.js#L461)（renderLayer addSource 失败标 failed）+ [shared.js:176](frontend/js/toolbox/shared.js#L176)（addToolboxLayer 检测）。**红线守约**：纯对账真值源增强（过滤条件）·不动 harness 主循环/分流/出口裁定条件·不改 diagnose prompt/schema。composePartialCard"未生成"语境（[:208](frontend/js/ai_qa/harness.js#L208)）涵盖渲染失败不需改。**验证**：node --check .mjs 语法过；运行时（飞轮渲染失败用例→partial 标注）留用户。commit 32924b9·**待用户 push**。
>
> - **5.208 EMC 专题 E2 · 进度透明（治 C9 延迟感知·setPhase 时间维度 + 阶段计时 + 增量落图 + 可见取消·纯前端零承重）**（用户"继续分支内容推进"·专题 E 三选一用户定 E2（全局 UX·中风险·所有问句受益）·主线程·未派 subagent）：**C9 痛点** = EMC 单问 10-40s·期间 UI 只静态"思考"chip·用户不知"还在跑/跑到哪/已多久"→ 焦虑误卡死。**现状**：[setPhase](frontend/js/ai_qa/panel.js#L903) 5 段 chip（诊断/思考/检索/生成/审查）有点亮 + hooks 时序完整 + abort（Esc/Ctrl-C）·但**无时间维度**（无耗时/计时/进度/增量落图/可见取消）。**改**（[panel.js](frontend/js/ai_qa/panel.js) + [ai_qa.css](frontend/css/ai_qa.css)·dock DOM 在 panel.js mountChatChrome 动态生成·非 index.html）：① **setPhase 加时间戳 + done 状态**（当前段 active + 已完成段 `.done` 填充·memory ramp-discrete-segments 离散分段）；② **阶段计时**（[_renderElapsed](frontend/js/ai_qa/panel.js#L931) 0.5s 刷新·"检索 8s · 共 12s"·_t0 总起始 + _phaseTs 阶段时间戳·治"已多久"）；③ **onAction 显"正在执行·[工具CN]"**（[_TOOL_CN](frontend/js/ai_qa/panel.js#L925) 映射 zonal_stats→分区统计 等·治"跑到哪"）；④ **onObservation 显"已生成 N 层"**（layer delta vs send 基线 _layerBase·增量落图·"图在长"）；⑤ **可见取消按钮**（dock `.aiq-abort-btn`·click delegation 复用 [_abortCtl.abort()](frontend/js/ai_qa/panel.js#L1282)·Esc/Ctrl-C 保留）。**红线守约**：纯前端 panel.js + css·**零承重**（不动 harness orchestrate / router.py / ChatRequest schema / diagnose prompt / SKILL_DEFS）·复用既有 hooks 时序（onAction/onObservation/onFinal）·**不加 onProgress hook**（panel 本地计时足够）。**设计语言**：填充式无线框（avoid-frames-fill-style）+ 胶囊紧凑（capsule-button-design-language）+ 离散分段（ramp-discrete-segments）。**验证**：node --check .mjs 语法过（ESM 假绿用 .mjs 副本）；运行时（start.bat 肉眼验进度条/计时/增量/取消）留用户（no-routine-playwright-verify·前端非控制流）。commit 4e15134·**待用户 push**。
>
> - **5.207 EMC 专题 D · diagnose 认知深化（D1 SOP 卡扩字段 / D2 method 确定性派生 / D3 EMC-SUM 计划→实产）**（用户"执行专题 D（diagnose SOP·eval-first·D1→D2→D3）"·plan `~/.claude/plans/plan-claude-plans-emc-gis-rippling-drea-whimsical-lobster.md`·主线程·未派 subagent·红线 eval-first·每次只改一处）：① **D1 SOP 卡扩字段**（[paradigm.py:170](ai_qa/paradigm.py#L170) GEO_TOOL_CATALOG 12 工具 +scale/preconditions/failure_modes·歧义工具 +examples 正负例 few-shot +[geo_tool_catalog_text](ai_qa/paradigm.py#L258) 渲染·[DIAGNOSE_TEMPLATE 本体](ai_qa/prompts.py#L168) 不动·**只扩附录数据**）：failure_modes 给 Flash 反向判据（clip 只切点勿面∩面→overlay / 单一空间关系勿选 multi / hotspot 逐点 Gi* vs density 连续面 / area_stats 面积非情绪）；[eval_template_flash.py](tests/eval_template_flash.py) +7 歧义用例。**eval-first 三跑**：基线 32/35=91% → 扩字段 31/35=89%（公园点位 rank 改善）→ rank/nearest 措辞微调 32/35=91%（**=基线不退化**·3 deterministic 改善固化：公园点位/各区排序/离地铁）。残余 3 MISS = LLM 方差 + 商业用地 select_template triggers 张力（[B_TRACK_PARADIGM](ai_qa/paradigm.py#L123) "区内的"→clip 过宽·面∩面语义张力·另案·非 catalog 范围）。② **D2 method 确定性派生**（[harness.js](frontend/js/ai_qa/harness.js) `deriveDiagnoseMethod(template,params)` + diagnose 解析后兜底·仿 R1-D1 deriveAvailable）：Flash 偶发输出 method·未输出时 single→[tool()] / multi→params.chain / concept→[]·元素 `tool()` 兼容 [_plannedGeoSteps F3 gate](frontend/js/ai_qa/harness.js#L382) 匹配。**解锁悬挂读取**：F3 完整性 gate（原 method 空·_planned=0<2 失效）+ [formatDiagnoseSummary](frontend/js/ai_qa/harness.js#L100) + _needsDeliberate 皆有源。③ **D3 EMC-SUM 计划→实产**（[e2e-seam.js:70](frontend/js/e2e-seam.js#L70) chatPhases +method / [test-cases.js](frontend/js/test-cases.js) sig.method+r.method / [test-board.js](frontend/js/test-board.js) _summary ②`计划N→实产M层` + EMC-SUM 头`计划命中=X/Y步` + md/json 报表 method 字段）：闭合 [test-board.js:26](frontend/js/test-board.js#L26) TODO（"需 diagnose method/plan 采集·后续批"）。**红线守约**：D1 只扩附录数据不动 DIAGNOSE_TEMPLATE 本体（eval-anchor 保）；D2/D3 不动 [ChatRequest schema](ai_qa/schemas.py)（method 留前端 trace·不回传·比 domain_lens threading 更轻）+ 不动 harness orchestrate 主循环（D2 派生在 diagnose 解析后·非 loop）。**验证**：D1 eval-first 3 跑 91% 不退化 + [test_a3_paradigm](tests/test_a3_paradigm.py)/[test_emc_template](tests/test_emc_template.py) 19 pass；D2/D3 三前端 JS .mjs 语法过（node --check .js 对 ESM 假绿·用 .mjs 副本）；运行时（?test=1 飞轮验 计划→实产 + method 渲染）留用户。commits ded4696/c074b75/02ec4ba/64244c8·**待用户 push**。
>
> - **5.206 toolbox 验收 + EMC 大收敛批次（步1 _contentSig / 步2 T3+T6 / 批次A T4+T5 / 批次C D1 扩覆盖·18 项全盘点）**（用户"今天 EMC 整体大收敛·不遗漏"·toolbox 工程验收 + 18 项全盘点 + 非红线批次落地·主线程·未派 subagent）：① **toolbox 验收**（[报告](.codebuddy/reports/toolbox-unified-acceptance-2026-07-26.md)·建议合并）：三件套本机复现全绿（obs diff 0/12 + unified ALL-PASS + pipeline ALL-PASS）+ 静态 A-E 全过 + 4 裁决（§7-4 互斥扩集接受 / M6 color 判据接受 K3 修正·**评审 distance 失误自认** / §6-7 area_stats 接受 / §7-3·§7-5 遗留）。② **全盘点 18 项**（emc-fix-backlog + K3 §9 + 验收 §四 + plan 步 1-6 交叉去重·不遗漏）。③ **步1 _contentSig 统一**（[main.js](frontend/js/main.js) import shared.toolContentSig·消重复·547a334）。④ **步2 T3+T6**（[test-cases.js](frontend/js/test-cases.js) 参数序列化 + hasAction 门控灭绝空心 OK·3d1e12b）。⑤ **批次A T4+T5**（[panel.js:816](frontend/js/ai_qa/panel.js#L816) strat 缺省 unknown 治胶囊矛盾 + [time-bar.js](frontend/js/time-bar.js) compare 无焦点提示·1fb9dfb）。⑥ **批次C D1 扩覆盖**（[harness.js:462](frontend/js/ai_qa/harness.js#L462) deriveAvailable 扩 request_upload + strategy 缺失·治 s1 残余 INT-003/004/006·eval 25/28=89% 不退化·62f25e7）。**批次B CPD 回归**：test_cpd_predicates failing（pre-existing 嫌疑·K3 §7-7 称未跑 + toolbox 不动 cpd-state/e2e-seam·登记另案）。**推迟**：A3 $n 链 diff（单工具 obs diff 已充分）+ ensure_zone registry（中间层 vs 结果层 provenance 专项）+ Layer Manifest（消费侧与专题 D 合并）+ P2。**专题 D/E 计划就绪**（diagnose SOP ⑥ + method→tool + EMC-SUM / harness D3 多步链 + P0-4 进度 + P1-4 partial 出口·红线 eval-first·后续会话各个击破）。**留用户**：T7 飞轮（验 D1 s1 收敛 + T4/T5 效果 + 干净基线）+ manifest 再生成 + DATA 迁移 commit。分支 toolbox-unified-toolset 19 commits 待合并（暂不合并）。**红线守约**：批次C 触 harness → eval gate 89% 不退化；diagnose prompt/ChatRequest 未触。commits 547a334/3d1e12b/1fb9dfb/62f25e7·**待用户 push**。
>
> - **5.205 EMC density 治本 3 包执行（C5 渲染兜底 / C6 认知 eval-first / C 分组+B srcId+T9·K3 "错位"根因）**（用户要求"先让 K3 出合成报告研判根因再推进"·K3 报告 [emc-eval-synthesis-2026-07-25.md](.codebuddy/reports/emc-eval-synthesis-2026-07-25.md)·主线程核验 K3 三承重判断属实 + 补两漏·**未派 subagent**）：**K3 根因升级"相位差→错位"** = 修复队列（系统内部健康序）与演示链（用户可见序）在 5.200-5.203 四批交集为空——改对了"病"没改"疼"；用户 5 投诉全落 5.204 plan 里。主因 (b) C5+C6 真实缺口 + (e) 验证债务；架构/红线无罪（c）。**核验 K3 属实**：time-source manifest 404（产品侧时间轴即坏·首报）/ C5 机制二分（weight 链只 seam 路径必然成立·[map.js:791](frontend/js/map.js#L791) coalesce 0.3）/ C6"密集"0 触发词（[paradigm.py:250](ai_qa/paradigm.py#L250)）；**补 K3 漏**：addHotpointLayer 同病（[map.js:742](frontend/js/map.js#L742)）/ buildWeight 兜底是 0.3 非 0.24（K3 数值小误·方向对）。按用户"省调动+高内聚低耦合"原则分 3 包：① **包A C5 渲染**（[map.js](frontend/js/map.js)·非红线）：`resolveWeightField`（数据有 emotion_intensity→用·缺→回退 score→uniform·同步 addHeatmapPaint+addHotpointLayer）+ renderLayer addSource try/catch + `_renderState` 标记。② **包C 低耦合小项**：C 分组（[tools.js](frontend/js/ai_qa/tools.js) density 委托后补 parentId→EmotionMap Copilot 组卡非空·用户#2）/ C7（[test-assets.js](frontend/js/test-assets.js) 行政区描述改正 9 要素无夷陵）/ B srcId（[tools.js](frontend/js/ai_qa/tools.js) `_toolContentSig` + addResultLayer/_registerToolboxLayer 按 srcId 去重·闭合用户#3 后半链）/ T9（[e2e-seam.js](frontend/js/e2e-seam.js) loadPoints 清旧 e2e_points·治堆叠加剧超时）/ time-source（MANIFEST_URL processed→performance·manifest 再生成留用户）。③ **包B C6 认知**（[paradigm.py](ai_qa/paradigm.py)·**红线 eval-first**）：density when 加"密集/集聚/热力图/情绪热度分布"触发词 + yields 改委托实况（彩虹热力/网格/地形）+ params 加 mode；[eval_template_flash.py](tests/eval_template_flash.py) 扩 2 例（"哪里最密集"/"情绪热力图分布"→density·**双命中**），重跑 **25/28=89% PASS**（3 MISS 均已知 Flash 路由歧义·非本批引入）。**承重纪律**：C6 触 diagnose prompt → eval-first（先扩例冻结→改→重跑验命中且不退化）；harness/ChatRequest 未触。**验证**：C6 eval 89% 已端到端验路由（diagnose flash 单轮）；C5 渲染防御性兜底（逻辑简单·demo 肉眼验彩虹图）；T7 飞轮全量重跑（04-07 干净基线·偿验证债务）留用户浏览器 ?test=1。**红线/未决**：DATA 迁移未 commit（用户红线）；manifest 再生成留用户；R1 残余 s1 4/9（D1 只盖 request_upload·T7 后若仍在扩覆盖·5.206 候选）。commit d8dbcb2·**待用户 push**。
>
> - **5.204 收工·06/07 评估 + density 治本 plan 定稿（不执行·留下会话）**（06 工具 pass=0% / 07 意图 pass=33% 3 OK 全空心·有效≈0%·EMC 本体核心未动；主线程派 3 Explore agent + 读 K3 报告 `.codebuddy/reports/emc-eval-report06-07-2026-07-24.md`）：**自评 + K3 互评**——"基本不可用"根因 = EMC 本体核心未修（测量层 4 批真进展，本体 C2/C3/C5/C6/C8 未动·K3 "相位差"成立）。3 agent 根因收敛：① **C5 渲染**（[map.js:685](frontend/js/map.js#L685) addHeatmapPaint `weightField=emotion_intensity` 默认·数据缺该字段→weight 表达式=0→rainbow 透明·用户#1 彩虹图不显示·最大见效）② **C6 工具认知**（"密集"触发词 5 处全空 + density 僵尸文案"方格面网格"（[paradigm.py:252](ai_qa/paradigm.py#L252)）+ Toolbox heatmap/grid/terrain 缺席 catalog → "不支持热力聚合"= LLM 幻觉·用户#4 密集→clip / #5）③ **C 分组**（[state.js:867](frontend/js/state.js#L867) `categoryOf` 不用 parentId·"EmotionMap Copilot"组卡永远空·用户#2）④ **B srcId 工具层**（addResultLayer 只按名去重·异名同内容堆叠·工具层无 srcId·用户#3 点名两次）⑤ T9 例间清层（e2e_points 堆叠加剧超时）⑥ C7 夷陵资产（行政区.geojson 9 feature 无夷陵区·EMC 判缺**对**·test-assets 描述错）。**density 治本 plan 已存** `~/.claude/plans/emc-gis-rippling-dream.md`（6 步·Step1 C5 渲染非红线最大见效 → Step3 C 分组最便宜 → Step2 C6 **eval-first 红线** → Step4 B srcId → Step5/6 T9/C7）。交接卡 [session-handoff.md](memories/repo/session-handoff.md) 已更。**红线/未决**：DATA/processed→performance 迁移是用户本地数据操作·**未 commit·数据红线·留用户处理**（我未碰）；C6 触 diagnose prompt → eval-first。后续（非本批）：D3 链式模板（C3 超时）/ T4 胶囊矛盾（C4·[panel.js:816](frontend/js/ai_qa/panel.js#L816) default ready）/ T6+T3 断言（C8 空心 OK）。本次 push 30 commit（29 旧 + 1 收工）。
>
> - **5.203 EMC 05-llm 修复 T1 + UI 固定图钉 + 排版/文风**（05-llm 四问题自评 + K3 对账后定方案；主线程 **未派 subagent**）：**自评对账 K3**——Q1-Q4 全命中（Q1 seam 洗坏+错池 / Q2 胶囊矛盾 [panel.js:816](frontend/js/ai_qa/panel.js#L816) default ready / Q3 对比 C键 批4 Swipe POC 退化 / Q4 信号-only 断言 + 区=[object Object]）；**补 K3**：T1 不只 washing，pool（`processed`→`performance`·processed 已空）+ 文件名（`xiling_wujia`→`yichang`·rename 漏改）也错。① **T1 修 seam**（[e2e-seam.js](frontend/js/e2e-seam.js) loadCSV + [test-assets.js](frontend/js/test-assets.js) + [import.js](frontend/js/import.js) export dsvRows·6f880a7·关键·解锁极性评估）：三修 → 真数据 16933 行·5 档充足（Very Neg 6610 / Very Pos 4716 / Neutral 3810 / Neg 1203 / Pos 594·非 ~89% Neutral）。**05/04 涉极性例结论作废·待 T7 重跑干净基线 + 验 R1**。② **UI 固定图钉**（[index.html](frontend/index.html) `#lp-pin` + [sidebar.css](frontend/css/sidebar.css) + [sidebar.js](frontend/js/sidebar.js)/[param-panel.js](frontend/js/param-panel.js) outside-click 守护·9fe6521）：Range/Layers/Toolbox 抽屉右上角图钉（Lucide thumbtack·品牌蓝 #4285F4 亮起），固定后点空白不隐抽屉 + param-panel（Esc/X 仍关）。③ **EMC 排版**（[ai_qa.css](frontend/css/ai_qa.css) + [panel.js](frontend/js/ai_qa/panel.js)·bc62e72·非红线）：问题理解卡字体 `2xs`→`xs`/`sm`（重要内容增大）+ 标签通俗化（软缺口·降级标注→部分数据替代·口径局限→部分数据用替代·结论会注明）。④ **答语文风**（[prompts.py](ai_qa/prompts.py) FINAL_TEMPLATE·284ae94·红线·飞轮验）：加「文风」指令（简短 3-5 句/生动具体/通俗优先·专业词紧跟白话/结论先），不碰承重规则（诚实/格式/出口）；eval 不覆盖 finalStep·无 eval gate·保守文风引导。**承重纪律**：3c 触 finalStep prompt 但保守（不改路由/出口/格式）·飞轮验；T1/UI/3a/3b 非红线。**验证**：T1 待 T7 重跑验极性分布；UI/3a/3b 手动可视；3c 飞轮答案文风。**路线**（[emc-fix-backlog.md](docs/emc-fix-backlog.md) 记录·状态跟踪）：T4（胶囊矛盾）/T5（对比 C键）/T6（断言完成校验）/T3（参数序列化）/D3（链式方法库·治 R2 真超时）pending。**下一步**：D3 或 T4/T5（你报的可见 bug）。
>
> - **5.202 EMC R1 数据认知治假 GAP（D2 可派生语义 + D4 grounding 枚举 + D1 派生判定器）**（04-llm[meta commit=58716bb·B1 后]17 例 pass=1(6%)：INT-002~007 西陵区假 GAP + INT-008~017 伍家岗/夷陵超时；K3 深化诊断 R1 双缺口；主线程 **未派 subagent**）：**根因（K3 深化·比我初判更准）** = R1 数据自检双缺口——① **strategy 语义缺口**（[prompts.py:210-211](ai_qa/prompts.py#L210-L211) 只教 hard-gap/fallback/ready·缺"超集可派生"类·LLM 按 :174"数据盘点要诚实"保守判 GAP）+ ② **可见性缺口**（grounding 不枚举 boundary 子要素名·diagnose 看不见行政区含西陵区）；工具层 zonal/compare 已能 `resolveBoundaryInput` 解析中文区名，**"工具能、数据在、认知层没告诉模型"**。R2 多步超时（INT-008~017·agent loop 逐步即兴规划）留 D3 下批。① **Step1 eval 扩区片例+冻结**（[eval_template_flash.py](tests/eval_template_flash.py)·f1ee84a·D2 红线 gate）：加西陵归因→zonal / 西陵的点→clip / 夷陵状况→zonal；多步「伍家岗哪里最差」Flash 路由漂移（multi/zonal/rank）即 R2 证据·留 D3。② **D2 strategy 可派生语义**（[prompts.py:210-211](ai_qa/prompts.py#L210-L211)·83b073b·红线 eval-first）：补"超集可派生→ready"强引导 + 例（引用 D4「含:」清单）；enum 保三值（derivable→ready·免下游 `strategy==='ready'` 耦合）；eval PASS 25/27=93% 不退化。③ **D4 grounding 枚举 boundary 全量名**（[tools.js](frontend/js/ai_qa/tools.js) buildContext·f77129b·非红线）：`_boundaryEnum`/`_boundaryNames` 对 boundary首选 层找 name 字段（categorical·≤20 unique·值含地名后缀 或 字段名似 name/MC）→ 全 distinct 值 → grounding 升级「行政区·含:西陵区/伍家岗区/夷陵区/…」（D2 燃料）。④ **D1 派生判定器**（[tools.js](frontend/js/ai_qa/tools.js) `deriveAvailable` + [harness.js:462](frontend/js/ai_qa/harness.js#L462) post-diagnose·37568f8·代码确定性）：扫 boundary name distinct 值 vs 问句匹配 → 命中即强制 `data_plan.strategy='ready'`（覆盖 request_upload·挡 :488 假 GAP 短路·Smart/Dumb 铁律"代码可知不问 LLM"·D2/D4 的确定性兜底）。**承重纪律**：D2 触 diagnose prompt → Step1 冻结 eval → 改 prompt → 重跑验不退化；D4/D1 非红线。**验证**：eval PASS（区片路由不退化）；**待用户跑飞轮** INT-002~007 验不再假 GAP（strategy=ready + zonal/clip 触发 + boundary=西陵区 resolveBoundaryInput 解析出图）；INT-008~017 多步超时留 D3。**下一步**：D3 链式方法库（extract→clip→density/grid→rank 确定性多步链·0 中间 LLM 轮）+ 多步 plan-once-execute + diagnose 出 chain_id（红线）治 R2 超时·覆盖 04 全部 17 例问型。
>
> - **5.201 EMC 治本 B0 护承重 + B1 模型路由（治超时#1·深度消化 K3 智能倒挂论断）**（K3 两份体验文档 review+plan 入库后，按其 B0→B1 序攻超时#1；主线程 **未派 subagent**——承重红线 eval-first）：**K3 深度论断** = 智能配置倒挂（Agent 做确定性工作付延迟税 / Tool 缺契约 dumb→fragile）+ 五脱节（①智能倒挂②数据认知③**延迟架构**④成功定义⑤测试价值）；本批攻③延迟架构错位。① **Step 0 入库 K3 文档**（[emc-experience-{review,plan}-2026-07-24.md](docs/emc-experience-review-2026-07-24.md)·9fd8dc4）。② **B0-1a eval 扩例+冻结**（[eval_template_flash.py](tests/eval_template_flash.py)·a93ce67）：加 compare×2（原缺·C 赛道区域对比）+ concept 负例×2 → **冻结基线 19/23=83%**（≥80% 解锁 B1-2b；4 MISS 是 Flash 路由歧义留 B3 SOP 卡治）。③ **B0-1b 词表 single-source**（[paradigm.py](ai_qa/paradigm.py)+[prompts.py:190](ai_qa/prompts.py#L190)·a96bfea）：新 `template_id_list_text()` 从 TEMPLATE_REGISTRY 派生 → diagnose 卡 enum 占位 `{template_ids}` 动态渲染（消 E9 三处词表漂移·加技能自同步）；重跑 **21/23=91% 无退化**。④ **B1-2a per-stage 路由**（[stages.js](frontend/js/ai_qa/stages.js)+[harness.js](frontend/js/ai_qa/harness.js)·d2cd5be）：finalStep/reviseStep → flash 默认，复杂任务（`_needsDeliberate`：strategy≠ready / method≥3）升 pro；砍 final+revise pro reasoner 串行（-20~40s）。⑤ **B1-2b 松 gate**（harness.js·78395c6）：`_TPL_HIT_RATE_GATE` 0.8→0.6（misses 仅计 unknown·0.6="Flash 真坏>40%"才退 while-loop·非"不完美"即退）→ single-skill 默认走 runTemplatePath（0 agent 轮·-20~40s）。⑥ **B1-2c 预算守卫**（harness.js orchestrate·1e49182·最深红线·单独 commit）：while-loop 顶 75s 总预算 → 超时 `narratedAnswer=true; break` → post-loop finalStep 出已执行结果（**保证必有回答·治 E1/E2 90s 超时无答**）。**承重纪律**：B0 触 diagnose prompt（词表）→ 先 1a 冻结 eval 再 1b 改 prompt 重跑验不退化；B1 触 harness（stages 路由+orchestrate gate/预算）→ B0 冻结基线为度量 gate，三子步按风险升序各独立 commit。**超时真因复盘**（坐实 K3 §2.1）：diagnose 已 flash，91s = agent 多轮 pro + final pro + revise pro 串行 → 2a/2b/2c 三刀（flash 化 + fast path + 预算网）合击。**验证**：eval PASS（83%→91%）；B1 待用户跑飞轮单工具例（density/zonal/rank）验 p95 下降（阶段目标<60s）+ 无 90s 超时无答 + 无质量崩（badge 非 gap 误判）。**下一步**：P0-4 进度+取消（UX·单列）/ B2 飞轮v5 / B3 P1 本体（GIS 工具 SOP 卡·治 4 MISS 路由歧义）。
>
> - **5.200 EMC P0 安全批·滚动复位 + srcId 去重 + density 执行信号**（治本工程第 1 批·安全 3 项·模型路由单列下一 plan；主线程 **未派 subagent**——承重边界内）：① **1a 滚动复位（E6·[panel.js:1260](frontend/js/ai_qa/panel.js#L1260) send）**——`_userPinned` 上滑置位后仅回底按钮复位 → 发新问不跟；send 入口复位 flag（appendMessage 已滚底 + autoScroll 续跟·ChatGPT/Claude 标准）。commit 89d7d70。② **1b srcId 去重（E3·[main.js:79-98](frontend/js/main.js#L79-L98) + runImport）**——addLayer 顺序发号零去重 → 重复上传堆叠；内容签名 `srcSig = 文件名 + featureCount + bbox + 前5feature 摘要`（**collision-free 串键·优于 hash·Map 精确匹配无碰撞**），`_srcIndex` 同 srcSig 复用 + 异名同内容关联 toast + 快照打 `layer.srcId`（**L001 编号零动**·srcId 供 EMC grounding 稳定引用·铺垫 P2 Dataset Registry）。commit ed1d97f。③ **1c density 执行信号（#2·[harness.js:332](frontend/js/ai_qa/harness.js#L332) + e2e-seam + test-cases）**——density 委托前端 Toolbox 无 fetch → 飞轮 `geoCalls` 正则抓不到 → TOL-001 永远 tool_hit=0；runTemplatePath 派发 `tool:executed{tool:skill,implTool,layerId,ok,ts}` 事件（**纯观测·不改控制流/出口/prompt·生产无监听者零开销·try/catch 兜底**）→ e2e-seam 监听入 `_testToolExecLog` + `toolExecs()` helper → test-cases 并集进 `sig.tools`（density 等前端委托工具不再漏）。commit 4a01052。**承重零触**（diagnose prompt / harness `orchestrate` 主循环 / ChatRequest 三承重不动；1c 派发在 runTemplatePath 子路径·非 orchestrate·纯观测 dispatch）。**验证**：语法肉眼复核（全 ASCII 标识符·无中文标识符 / 无 MapLibre query 改动·node 不在环境无法 .mjs 校验）；待用户肉眼验 1a（滚上→发新问自动跟到底）+ 跑飞轮 TOL-001 验 `sig.tools` 含 density（tool_hit>0）。**下一步**：模型路由(超时#1·harness 红线)独立 plan + 先扩 `tests/eval_template_flash.py`。
>
> - **5.199 测试飞轮两批修复入库 + EMC 治本 backlog 起步**（测试飞轮两批治标验证——H1 生效 + pytest 203 pass——后定调"逐个治标无解 → 转治本 K3 P0/P1/P2"；主线程 **未派 subagent**——承重）：① **入库两批改动**（13 文件·commit a90fac1·**待用户 push**）：**批1 信号链** H1 template 信号接通（[panel.js:1090](frontend/js/ai_qa/panel.js#L1090) onDiagnose dispatch `diagnose:done` + [e2e-seam.js:56](frontend/js/e2e-seam.js#L56) chatPhases 听事件·治 C1 断链「ChatRequest 无 diagnose 字段致 tmpl 断言永 false」）/ H3 参数断言硬化（cell/radius/boundary 容差匹配·10 例不再恒 pass）/ H5 JSON 报告 sidecar（run 元数据 commit/时间/配置）/ EMC-SUM v1 3 行摘要（链路/产物/状态 键值定序）；**批2 覆盖** A 字段识别扩容（DataEye 关键字段全值·非 6 字段×2 样本×24 字符）/ B 摘要 3 行中文 / C 渲染断言（mapSources + renderedNew·捕"图层入 state 地图空"2a 场景）。② **治本 backlog** [docs/emc-fix-backlog.md](docs/emc-fix-backlog.md)：6 类问题（超时#1/density 信号#2/选错工具#3/字段 manifest#4/渲染 bug#5/摘要#6）+ 证据 + 文件行号 + K3 P0/P1/P2 方向。③ **本会话纠正 K3 §3.1 过时认知**——K3 说"diagnose Pro→Flash"已**过时**，diagnose 实跑 flash（[stages.js:236](frontend/js/ai_qa/stages.js#L236)）；超时真因 = agent 多轮（[:217](frontend/js/ai_qa/stages.js#L217) `ctx.model` 默认 pro）+ final（[:251](frontend/js/ai_qa/stages.js#L251) pro）+ revise（[:296](frontend/js/ai_qa/stages.js#L296) pro）**串行**。④ **P0 切分拍板**（用户定·安全 3 项优先）：模型路由(超时#1·触 harness orchestrate 主循环红线)单列独立 plan + 先扩 `tests/eval_template_flash.py`；本批先做安全 3 项（滚动复位/srcId 去重/density 执行信号·均不触三承重）。**承重零触**（diagnose prompt / harness orchestrate / ChatRequest 三承重不动·改动全在 ?test=1 测试抽屉 + serve dev 层 + DataEye 观测）。pytest 203 pass 基线稳。
>
> - **5.198 _fill 中文占位符修复 + todo 机制反思 + 测试报告入库**（用户实测发现意图 prompt 全显 `{区}`；主线程）：① **根因**——[test-cases.js](frontend/js/test-cases.js) `_fill` 正则 `/\{(\w+)\}/g` 的 `\w`=[A-Za-z0-9_] **不含中文** → `{区}`/`{要素}`/`{用地}` 全未替换（200 例 prompt 失效；**语法绿·计数对·仅输出扫描能查出**）。② **全局审查** 4 文件所有正则（test-assets/test-cases/test-board/e2e-seam）：仅 _fill 一处同类（其他处理 ASCII URL/markup 或用中文 literal）。③ **修** `\w+`→`[^}]+`，运行时扫 270 例 0 残留。④ **memory 防复发** `js-regex-word-chinese-trap`（正则 \w 不匹配中文·审查清单·验证习惯）。⑤ **todo 机制反思**——bug 修复 5 轮未调 TodoWrite → 用户发现 todo 停滞 → 证实「执行密集时忘同步」根因 → 用户级 CLAUDE.md「Todo 生命周期」规则依赖自觉、效力有限，**最可靠是用户见停滞即催**。⑥ **测试报告 [tests/reports/](tests/reports/) 入库 git 同步**（用户换环境要用，勿 gitignore）。commit·push。
>
> - **5.197 测试飞轮 v4·意图识别方向纠偏 + 意图/工具各 100 例 + DATA 资产系统 + 按钮状态机 + todo 机制**（用户实测 v3 后提 6 点（含「意图识别方向错了」）+ 用户重组范围文件；主线程 **未派 subagent**——承重）：① **意图识别方向纠偏（核心·痛点3）**——意图 ≠ 回答文本，而是 **NL→可执行工作流转译**（范围→筛选→分支→工具→步骤→执行）；EMC 已具此能（diagnose 选 template→harness 跑工具链）；测试断言转向 **template+工具转译链**（非答案文本·[test-cases.js](frontend/js/test-cases.js) `_assertIntent`）。② **痛点1 DATA 资产识别**——新建 [test-assets.js](frontend/js/test-assets.js)（语义清单 RANGES/POINTS：`'行政区'`/`'二马路片区'`/`'L2-T1'`→实际文件）；llmRun 语义加载不再让用户补范围；用户**重组 boundaries**（presets/ 上移顶层 + manifest 移位），修 test-cases 原 `presets/行政区.geojson` 404。③ **痛点2 摘要极简关键**——llmRun 抓 template/params/newLayers，`_summary` 分型（工具类 `tools:zonal_stats · 区=西陵 · +1层`；意图类 `tpl=zonal · tools:...`）。④ **痛点4 用例重写**——生成器：工具类 100（14 GIS 工具针对性·硬断言 `tools.includes(expectTool)`·≤2 工具≤4 步）+ 意图类 100（6 型 × 区/pair/r 轴·软断言转译链）；总量 270；setup 弹窗数量改 **slider 默认 25**。⑤ **痛点5 存报告覆盖确认**——手动存若本 session 已存→confirm 覆盖（[serve.py](frontend/serve.py) `/_test/report` 支持 name 覆写·不 +1）。⑥ **痛点6 按钮状态机**——[test-board.js](frontend/js/test-board.js) v4 主按钮 停止↔重新开始（连贯多场景）+ FAB 始终可重配 + 重跑中断批量。⑦ **todo 机制固化**——用户问「todo 不更新根因」→诊断（模型执行时忘同步 + 列表长倾向批量更新·**非工具 bug**·调动计费下对调用数影响小，主要伤 token/上下文）→ 用户级 [CLAUDE.md](../../Hi/.claude/CLAUDE.md) 新增「Todo 生命周期（定期归档/删除）」节（完成即归档/阶段覆盖/活跃≤6/WIP=1/关键节点更新·跨项目）。**承重零触**（业务代码/diagnose/四态出口/harness 全不动）。node --check `.mjs` 绿 + py_compile 绿 + 运行时用例计数 INTENT 100/TOOLS 100。commit·push（用户明确）。**下一步**：v4 实测收集转译断言失败→调 prompt 池提 pass 率。
>
> - **5.196 测试飞轮 v3·用户实测 4 痛点 + start.bat 一键启动**（用户实测 v2 后提 4 痛点；主线程 **未派 subagent**——承重）：① **痛点3 行内摘要+工具标注**（[test-board.js](frontend/js/test-board.js) v3）：每行常驻摘要行（两行布局），工具类直接标 `tool: zonal_stats`（从 [e2e-seam.js](frontend/js/e2e-seam.js) fetch 拦截的真实 /geo+/spatial 端点抓·非猜），其他类型回退 obs；**修 v2 死代码** `.tb-extra` 永不可见（无人 toggle tb-expanded）→ 改 `.tb-summary` 常驻；[test-cases.js](frontend/js/test-cases.js) llmRun 抓 `result.tools`；[e2e-seam.js](frontend/js/e2e-seam.js) geoCalls() 扩含 /spatial/（不漏 grid）。② **痛点2 重跑 R 没反应**：根因 `_rerunOne if(_running)return` 批量中静默 → 修为「先停批量→等当前例完→重跑」（toast 反馈·R 永远有响应）。③ **痛点1 固定位置报告**：[serve.py](frontend/serve.py) 加 dev-only `POST /_test/report` → 落盘 [tests/reports/](tests/reports/) `report-<日期>-<编号>-<类型>.md`（编号同日自增·跨会话唯一）；停止/完成自动存 + 手动「存报告」钮；报告含工具列+待复查清单。④ **start.bat 一键开主页+测试页**：[serve.py](frontend/serve.py) 加 `--open=both` 标志（`_open_browser` 后台 webbrowser 线程·socket listen 后开两标签·非 batch 脆弱 PS 轮询）；[start.bat](start.bat) 改 `--open=both`；[README.md](frontend/README.md) §一 加方式 A（双击 start.bat）+ 测试页 URL `?test=1`。**承重零触**（业务代码/diagnose/四态出口/harness 全不动·改动全在 ?test=1 测试抽屉 + serve dev 层 + 启动器）。node --check `.mjs` 绿 + py_compile 绿 + 无中文标识符。commit·**待用户 push**。
>
> - **5.195 测试飞轮 v2·100 例 + 8 项 UX 改进**（用户实测 v1 后提 8 项改进 + 要求 100 例；主线程 **未派 subagent**——承重）：**8 项 UX 改进**（[test-board.js](frontend/js/test-board.js) 重写）：① LLM 例自动展开 EMC（focus→setEmcCollapsed(false)）；② [e2e-seam.js](frontend/js/e2e-seam.js) 加 `loadCSV`（fetch /DATA CSV→解析→GeoJSON→loadPoints）+ `loadRange`（fetch /DATA/boundaries→addLayer polygon）→ 真实数据测试；③ 点击用例行→EMC 跳转至该答案（scrollIntoView + 高亮闪烁）；④ 每例 🔁 重跑按钮（单独重跑·newChat 清上轮）；⑤ 设置弹窗（🧪→弹窗：模式 no-llm/llm/全 + 类别多选 + 数量/超时限制→开始）；⑥ 按钮状态修复（运行中 🧪 变灰 spinning·开始/停止互斥）；⑦ 每例 👍/👎 满意度（用户判断**覆盖自动**：👎→fail 即使自动 pass）；⑧ **100 例**（[test-cases.js](frontend/js/test-cases.js) 重写·data-driven 生成器 × 8 类别：CPD 导游 15 / UI 渲染 15 / 引擎谓词 10 / 意图识别 15 / 工具选择 15 / 参数正确性 10 / 成果范式 10 / Smart 交流 10 = 100）。[test-board.css](frontend/css/test-board.css) 加弹窗 + 重跑 + 满意度 + 跳转高亮样式。**承重零触**（不改业务代码·?test=1 only）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。
>
> - **5.194 测试自动化飞轮 MVP·项目网页端测试抽屉**（用户要求"不另开炉灶·项目网页端·实时观测·分环节分类·fail fast·报告归纳"；主线程 **未派 subagent**——承重）：**飞轮 = bug 报 → 登记用例 → 脚本 → 网页端抽屉实时跑 → 导出报告 → 第三方根因**。本次 MVP 5 文件：① [e2e-seam.js](frontend/js/e2e-seam.js) **扩展 test helpers**（send/waitAnswer/badge/collapsed/welcome/hintVisible/guidanceCard/layerNames/...·20+ DOM 断言方法）+ **fetch 拦截**（monkey-patch window.fetch → 日志 /chat + /geo 供分阶段断言）；② [test-cases.js](frontend/js/test-cases.js) **12 用例**（T01-T05 no-llm：空态欢迎/hint 消失/卡片互斥/Pro-Flash 图标/方向级联 + T06-T11 llm：zonal 不误判 GAP/compare 产层/网格→3d/通用问答/缺参 ask/GIS 裁剪 + T12 Pro/Flash 切换·每例 `{id,name,category,type,run:async(t)=>{pass,stage,obs,review?}}`·fail fast 阶段断言）；③ [test-board.js](frontend/js/test-board.js) **抽屉 UI + runner**（浮动 🧪 按钮 → 右侧抽屉·模式 no-llm/full · 用例列表 pending→running→pass/fail/review 实时更新 · 进度统计 · 🟡 review 👍/👎 交互 · 导出 markdown 报告）；④ [test-board.css](frontend/css/test-board.css)（design tokens·Light/Dark）；⑤ [index.html](frontend/index.html) `?test=1` dynamic-import（加载 e2e-seam→test-board·生产永不加载）。**LLM**：GLM 0 次（纯前端 JS）；DeepSeek 按 mode（no-llm=0 / full=~50-100）。**承重零触**（不改业务代码）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。bug-log + catalog 扩展留 follow-up。
>
...
>
> - **5.191 EMC 两 bug 修复·滚动跳顶端 + 字段失败 Smart 恢复（用户报"EMC 退化"两 bug）**（主线程排查 + Playwright 验证 **未派 subagent**——承重）：① **Bug 滚动跳顶端**（[panel.js](frontend/js/ai_qa/panel.js) `_fitEmcToContent`）：根因 = 量自然高时**临时 `msgs.style.height='auto'`** → 滚动容器撑满无溢出 → **scrollTop 钳为 0**（浏览器不保留旧值）→ `_setupEmcContentFit` MutationObserver（characterData）**每流式 token 触发 `_scheduleFit`** → 流式期一直跳顶（autoScroll 底 与 fit 顶 交错抖动）。修：量前 `savedTop = scrollTop`、量后 `scrollTop = savedTop` 恢复。② **Bug 字段臆造 + 失败直接 GAP**（[harness.js](frontend/js/ai_qa/harness.js) runTemplatePath）：LLM 臆造字段（如 MC·应用 name·diagnose prompt 不动）→ `[ERR] 字段不存在` → 硬 ERR → **直接 EXIT_GAP**（无 Smart 恢复·"丢失 agent 灵活性"）。修：失败分类——**可恢复失败**（obs 含 `字段不存在|可用:|缺.*槽|无可见点|未找到|无结果|无匹配`）→ **ask_user**（问题含失败 obs + 可用字段 + 引导选项「我来指定正确字段/换方向/看现有」）；**硬 ERR**（网络/异常）保留 GAP。守 Smart「失败时交流、不猜不放弃」。**Playwright 验证**：流式期 scrollTop 恒贴底（1159·nearBottom=true·**jumpedToTop=false**·不跳顶）。**承重零触**（diagnose prompt 不动·字段臆造是 LLM 质量靠 ask 反馈兜底；四态出口不动·ask 是既有出口）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。
>
> - **5.190 补 v1.4 边界·compare 产合并聚合图层 + P2 ask_user 扩展**（用户确认「补 v1.4 边界」；主线程 **未派 subagent**——承重）：补 v1.4 两处边界。① **compare_regions 产合并聚合图层**（[tools.js](frontend/js/ai_qa/tools.js)）：循环存 `boundaryGeo`（resolveBoundaryInput 结果）+ 新 `_compareToLayer`（复用 `_buildZonalFc` 逐区合成 → 合并多区 polygon 为一个 FC → `addResultLayer _ui.tool='zonal'` choropleth 红绿）→ return `data.layerId` → **多区红-绿地图** + activeAnalysis 可认（深读胶囊在 compare 场景也能工作）。② **P2 扩展 missing-slot→ask_user**（[harness.js](frontend/js/ai_qa/harness.js) runTemplatePath）：新 `_missingSlotAsk`（缺 boundary→"分析哪个区？"+宜昌区精准选项 / center→设施名 / range/layer/target 各有引导）→ 缺必填槽时**提问非 GAP**。③ **P2 扩展 _hardFail→ask_user**（while-loop 零成功·harness.js:562）：提问（换问法/上传/看现有·问题带失败工具名）非直接 GAP 放弃。**Playwright 验证**：注入点层 + "对比西陵区和伍家岗区"→ badge「已生成 1 个图层」（exit=result·不再误判 GAP）+ **图层「对比·西陵区·伍家岗区」**（合并聚合层产出）+ 正常对比结论（"两区极性 0.33..."）。**承重零触**（diagnose prompt/四态出口不动；P2 用既有 ask 出口）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。
>
> - **5.189 EMC 误判 GAP 核心修复 + zonal 产聚合图层 + Smart ask_user + Pro/Flash 图标（v1.4·全套·用户长期 bug）**（用户长期困扰：数据齐全却"未产出图层"→误 EXIT_GAP→续作半成品→深读胶囊死链；要求详细排查根因 + 客观判断能否解决；主线程排查 + Playwright 验证 **未派 subagent**——承重）：**根因 A（核心）** = `zonal_stats`/`compare_regions`/`rank`/`area_stats` 是**表格型分析工具**（返 `data.rows`、无 `data.layerId`·[tools.js:723](frontend/js/ai_qa/tools.js#L723)），但 `runTemplatePath` 成功判定**只认产图层**（[harness.js:289](frontend/js/ai_qa/harness.js#L289) `if(r.data.layerId) newLayerCount=1`）→ zonal 即使 rows 非空、数据齐全也被 `newLayerCount===0` **误判 → EXIT_GAP**（"未产出图层"）。**根因 C** = 深读（inspect/deep_read）依赖 `activeAnalysis()`（grid/terrain 聚合层），zonal 不产层→深读"暂无聚合层"→胶囊死链。**客观结论：能解决**（核心 A 明确可修）。① **P0 修误判**（[harness.js](frontend/js/ai_qa/harness.js)）：加 `_ANALYTICAL_TOOLS` 集 + `hasRows`（分析型 rows 非空=成功）→ zonal 不再误判 GAP。② **P1 产聚合图层**（[tools.js](frontend/js/ai_qa/tools.js) `zonal_stats`）：`_zonalToLayer`（rows + boundary geojson → 合成 polygon，`piToNorm` 注 `_grid_norm`，`addResultLayer _ui.tool='zonal'` 复用 grid choropleth 着色）→ 产红-绿聚合地图 + `isAnalysis` 认 'zonal'（深读可工作）；[map.js](frontend/js/map.js) `addPolygonPaint` isTool 加 'zonal'。③ **P2 Smart ask_user**（harness）：失败分支**非硬 ERR → ask_user 提问**（换范围/上传/全域·精准选项·不直接 GAP 放弃·守 Smart Agent「不确定时交流」）。④ **UI**：Pro/Flash 文字 → **大脑/闪电 SVG**（[index.html](frontend/index.html)+[ai_qa.css](frontend/css/ai_qa.css)·设计语言统一）。**Playwright 验证**：注入 L2 点层 + 发"西陵区情绪归因分析"→ **badge「已生成 1 个图层」（exit=result·不再误判 GAP·之前是"缺数据·需上传"）** + 答案「已生成 西陵区_情绪聚合 图层…落图…整体偏向积极（极性 0.33）」（P1 聚合层产出 + 正常结论）+ UI 图标渲染。**承重零触**（diagnose prompt/四态出口不动；P2 用既有 ask 出口）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。
>
...
>
> - **5.184 CPD 导游多阶段（阶段 A 大方向分类 + 阶段 B 细化追问·Step 1b）**（用户重新梳理 CPD+EMC 工作流：CPD=导游·确定性·不思考/不派发，EMC=agent·Flash收集+Pro研判/计划+执行；主线程 **未派 subagent**——承重）：落实 9 阶段工作流的 CPD 导游部分（阶段 A/B）。① **[cpd-guide.js](frontend/js/ai_qa/cpd-guide.js)** row 7 analyze 改 `{kind:'intent', directions, refinements, ctaKind:'analyze'}`——**阶段 A 4 类大方向**（情绪分析/GIS操作/周边分析/深读区域·`_DIRECTIONS`·每类映射一组 SKILL）+ **阶段 B 细化追问**（`_REFINEMENTS`·每方向多分支追问胶囊·确定性·不调 LLM）；interpret（hasAnalysis）保留 examples（读图）。删 `_EXAMPLES_ANALYZE`（具体问句→改方向级联）。② **[panel.js](frontend/js/ai_qa/panel.js)** `_renderGuidanceContent`（intent=方向级联 / interpret=examples / 其余清）+ `_renderDirections`（label「方向」·点击记 `_curDirection`→阶段 B）+ `_renderRefinements`（label「细化」+「‹ 返回方向」·点击→send→harness）；cpd:guidance 监听 + `_runGuidanceCta` analyze + clearSuggest（reset `_curDirection`）统一接 `_renderGuidanceContent`。**CPD 只指方向（确定性），意图识别归 EMC Flash diagnose**（铁律3·用户选大方向/细化→send→orchestrate→diagnose）。**Playwright 复测**：mock intent guidance → 阶段 A 4 方向胶囊 → 点 emotion → 阶段 B 细化胶囊（全域/哪个区）+ 返回按钮。承重零触（diagnose/四态出口/harness 不动）。node --check `.mjs` 绿 + 无中文标识符。commit·**待用户 push**。**下一步 Step 2**：diagnose 回传 + 阶段 D 参数引导（template→SKILL_DEFS）→ Step 3 Pro 研判 deliberateStep。
>
> - **5.183 CPD hint 消失 bug 修复（suppress 仅折叠态生效·Step 1a）**（用户报「EMC 中 CPD UI 不显示」；Playwright 实测定位根因；主线程 **未派 subagent**——承重）：**根因**（实测确认）= halo 点击 → `suppressGuidance` → 下次 `_compute`（layers:changed 等触发）将 guidance 置 null（suppressed）→ **展开后 `_curGuidance` 仍 null** → `.emc-cpd-hint` hidden=true/display=none（实测 afterExpand hintHidden=true）。**修**：① [cpd-guide.js](frontend/js/ai_qa/cpd-guide.js) `_compute` suppress **仅折叠态生效**（加 DI `isCollapsed()` 判断·展开态提示条始终反映当前引导·光环才需 engage 解除）；② 新增导出 `refreshGuidance()`（仅 `_compute`·不 reset 抑制/去重）；③ [panel.js](frontend/js/ai_qa/panel.js) `setEmcCollapsed` 末尾 `_applyGuidance()`→`refreshGuidance()`（折叠↔展开重算·展开后 hint 反映当前引导）+ DI 注入 `isCollapsed: () => _emcCollapsed`。**Playwright 复测通过**：halo click + suppress + 触发 _compute + 展开 → hint 正常显（hintHidden=false / display=flex / text=range 文案）；折叠态 suppress 仍生效（光环不重亮）。**承重零触**（diagnose/四态出口/harness 不动；CPD 纯确定性）。node --check `.mjs` 绿。commit·**待用户 push**。**下一步 Step 1b**：CPD 多阶段导游（阶段 A 大方向分类 + 阶段 B 细化追问）→ Step 2 diagnose 回传 + 阶段 D → Step 3 Pro 研判 deliberateStep。
>
> - **5.182 CPD 引导重做·双域架构（EMC↔CPD 互不干扰）+ 空态欢迎语**（用户不满引导体验要求重做 + 重新定义 EMC/CPD 双域关系；进 plan 模式确认两选择；主线程 **未派 subagent**——承重）：**用户重新定义** EMC 与 CPD = **独立两域互不干扰**——收起 CPD 接替（光环胶囊提示下一步）/ 展开 EMC 接手（对话/agent 详细解释）+ CPD **缩小为提示条**继续提示。三诉求：① 空态恢复欢迎语（不一上来引导操作）；② 双域 UI（展开 CPD 提示条·进度点上方·去光环·阴影·Light/Dark）；③ 意图 agent 轻量。**两确认选择**：意图 agent = **轻·复用现有 harness**（不改承重·展开显确定性引导·用户首次输入才触发 LLM·聚合参数由 ask_user 问）；装载链引导**保留**。① **[cpd-guide.js](frontend/js/ai_qa/cpd-guide.js)** §4.2 row 2 `!hasImport→null`（删 import 引导·空态显「向 Copilot 提问…」欢迎语·点开折叠→欢迎卡；导入后才入装载链 row 5+）。② **双域 UI**（[panel.js](frontend/js/ai_qa/panel.js)+[ai_qa.css](frontend/css/ai_qa.css)）：`.emc-cpd-hint` 提示条（`_setupCpdBar` `bar.prepend` 进度点上方·随 `.emc-cpd-bar` 折叠态 `display:none` 自动隐·仅展开态显）+ `_applyCpdHint`（填 guidance.text+data-cta）+ `_applyGuidance` 扩展（折叠光环/展开提示条**分治**）+ 点击 hint→`_runGuidanceCta`；CSS 卡片式（`accent-soft` 填充·无线框守 avoid-frames·阴影加深 `0 2px 8px`·缩小 11.5px/600·Light/Dark 走 theme var 自动·`[hidden]` 显式覆盖 `display:flex` 保隐藏生效）。③ **[cpd-core-plan.md](docs/cpd-core-plan.md) v1.2**：§4.2 row 2 + §决策3 双域架构 + 顶部 v1.2 注记（不开 CB-CPD-04）。**承重零触**（diagnose/四态出口/harness/tracker 不动·意图识别归 EMC Smart Agent；CPD 纯确定性·不调 LLM）。examples（#aiq-suggest·EMC 对话起点）与 hint（CPD 提示·顶部）分域并列。node --check `.mjs` 绿 + 无中文标识符。**待用户肉眼验**（空态欢迎语→点开欢迎卡；导入点+范围→展开显提示条+examples；Light/Dark 切换配色）。commit·**待用户 push**。
>
> - **5.181 CPD analyze 多分支→对话交接闭环（plan v1.1 修订·实施发现 plan 层新问题）**（用户反馈 analyze 阶段引导过窄 + 建议对话收集意图；进 plan 模式确认两设计选择；主线程 **未派 subagent**——承重）：**洞察** = 点+范围就绪后下一步分支爆炸（gis操作 × gis分析 × 操作→分析），**确定性引导穷举不了**，硬给固定 CTA 卡用户 → 改**主推对话收集意图**（plan §决策2 接缝：CPD 确定性交接 → EMC harness 意图识别）。**严格守 EMC 内核**（用户承重提醒）：CPD 引导层 = 确定性编排器（`deriveGuidance` + 确定性 examples 常量·**铁律3·不调 LLM**）；意图识别归 EMC Smart Agent（[diagnose](frontend/js/ai_qa/harness.js)·已有·不动）；**CPD 只"交接"不"识别"**。① **[cpd-guide.js](frontend/js/ai_qa/cpd-guide.js)** analyze/interpret 加 `examples`（确定性常量 `_EXAMPLES_ANALYZE`/`_EXAMPLES_INTERPRET`：情绪分析/GIS操作/周边分析/深读区域·覆盖三类分支 + 空间深读）+ 文案「数据已就绪——告诉我你想分析什么」。② **[panel.js](frontend/js/ai_qa/panel.js)** `renderGuidanceExamples`/`clearGuidanceExamples`（复用 `#aiq-suggest` + `.aiq-suggest-chip` 样式·零新 CSS）+ cpd:guidance 派发 + CTA analyze 展开 input + 显 examples + **互斥**（`_shouldShowGuidanceExamples`：仅首次分析前显，有答案 `_followUps` 接管 + `_guidanceExamplesShown` 标志防误清追问）。③ **[cpd-core-plan.md](docs/cpd-core-plan.md) v1.1 修订**：§4.2 row 7（多分支→对话交接 + examples）+ §6.4 S3（空间交互降为 examples 一项·用户定）+ 顶部 v1.1 注记（不开 CB-CPD-04）。**两设计选择**（用户定）：① 对话窗口 = 复用 EMC 输入框（展开 input + examples·不造新弹窗·守软折叠基调）；② 空间交互（点击地图深绿/深橙）降为 examples 一项。**承重零触**（diagnose/四态出口/harness/tracker 不动；CPD 纯确定性·意图识别归 harness）。node --check `.mjs` 绿 + 无中文标识符。**待用户肉眼验**（导入点+范围→折叠文案变→点光环展开→显 4 示例胶囊→点示例走 harness）。commit·**待用户 push**。
>
> - **5.180 CPD G1 引导链贯通修复（导入/范围上载漏 dispatch + range 文案）**（用户反馈三问题排查；主线程 **未派 subagent**——承重）：① **根因 = [main.js](frontend/js/main.js) `runImport`/`runRangeImport` 上载点层/范围后只 `renderLayerList()` 漏 dispatch `layers:changed`**（框选 [draw-tool:130](frontend/js/draw-tool.js#L130) 有 dispatch，故框选能推进引导、上传不能）→ cpd-guide 引擎不 recompute → guidance 卡在光环 CTA 点击态（默认 placeholder）= 用户报「导入点层文案不变 range」「点+范围上载后无进一步引导」**同一根因**。修：两处末尾补 `dispatchEvent(layers:changed)`（引擎 recompute + popup/Overview/legend/compare 监听者统一刷新；`renderLayerList` 已直调，listener 再调幂等）。② **range 文案**「框选范围」→「上传范围文件，或在地图框选」（range drawer 已含 预设/手绘/上传 三方式，CTA 打开 drawer 即让用户选，文案补全两种主要方式）。③ **用户建议「断连→意图识别 Agent(LLM)→plan→重连」未采纳**——违背 plan v1.0 铁律3（引导层 = 确定性编排器·不调 LLM；diagnose 是对话路径职责·§决策2 分工）；断连根因是**工程 bug 非设计缺陷**，修 dispatch 后链贯通（import→range→layers/analyze）；若要"智能推进"，正确入口是用户主动问 EMC（走 harness diagnose·已在），非引导层内嵌 LLM。承重零触（diagnose/四态出口/harness 不动）。node --check `.mjs` 绿。**待用户肉眼验**（导入点层→文案变 range；点+范围→analyze 引导）。commit·**待用户 push**。
>
> - **5.179 CPD P2·G1 引导引擎 MVP + 折叠耦合 + 光环可点**（plan v1.0 §八 P2 G1；主线程 **未派 subagent**——承重）：curState 从「反映」升「编排」——新增确定性引导引擎，纯函数从客户端特征向量算「此刻唯一动作」（[CLAUDE.md「AI·Copilot 开发内核」](../CLAUDE.md) 铁律3·编排器确定性不调 LLM）。① **新建 [cpd-guide.js](frontend/js/ai_qa/cpd-guide.js)**：`deriveGuidance()` 特征向量真值表 `(hasImport, hasRange, hasVisibleEmotionLayer, hasAnalysis, lastExit, streaming)` → `{kind,text,ctaKind}|null`（9 行首匹·streaming 第一→import 先导→gap/partial/ask 让 `_followUps`→drift→range→layers→analyze/interpret(M1)→export）；依赖注入 `initCpdGuide({getLastExit,isStreaming,getLastRegion})`（**零 import panel.js**·CB-CPD-02 H1 消循环 import）+ 单调去重 `turnId>_lastTurnId`（**H1 修 general 断链**·CB-CPD-03）+ engage 解除 `suppressGuidance`（同 kind 不重亮·plan §6.2.3）。② **[cpd-state.js](frontend/js/ai_qa/cpd-state.js) 导出 4 谓词** hasImport/hasRange/hasAnalysis/hasVisibleEmotionLayer（**M2 收紧**：判情绪性 = colorMode l2- ∨ 聚合工具 ∨ feature 极性字段，否则对无情绪层撒谎「点击深绿/深橙」演示链第一环断点）+ S4 信号 `.aiq-conclusion`→`.aiq-exit-badge`（死信号修正·CB-CPD-01 K3 P0-1）。③ **[panel.js](frontend/js/ai_qa/panel.js)**：`send()` finally dispatch `cpd:turn-ended {exit,turnId,intent}`（**`settled` 守卫**覆盖 general exit=null + 单调去重·H1 修）+ 删 Ctrl+Shift+G 测试接引擎驱动 + 光环可点 CTA（`.emc-input-area` click→import/range/layers `cpd:focus-tab` / analyze·export 展开 input）+ engage 解除 + `clearChat()` recompute（切会话恢复）。④ **[sidebar.js](frontend/js/sidebar.js)** `cpd:focus-tab` 扩 `import`→`openImport`（1 行·G1 import CTA 闭环）。⑤ **[ai_qa.css](frontend/css/ai_qa.css)** 光环 7 色 hex→`--emc-halo-*` theme var（dark `:root` + `[data-theme=light]` 双主题·Light 校准·CB-CPD-01 K3）。⑥ **[e2e-seam.js](frontend/js/e2e-seam.js)** 暴露 `window.__cpdPredicates`（mirror `__emcTest`·main.js 零 test 代码）。⑦ **测试**：[fixtures/plain_poi.geojson](tests/browser/fixtures/plain_poi.geojson)（无情绪字段测 M2）+ [test_cpd_predicates.py](tests/browser/test_cpd_predicates.py)（用例 10·M2 硬断言·免 LLM）+ [test_cpd_guide_no_freeze.py](tests/browser/test_cpd_guide_no_freeze.py)（用例 11·H1 确定性 dispatch 模拟免 LLM）+ [emc-test-cases.md](emc-test-cases.md) 用例 10/11 ⬜→🔄。**承重零触**（diagnose/四态出口/harness/tracker/curState 推导逻辑全不动；引擎只读 + 事件松耦合 + 依赖注入单向）。三坑过（node --check `.mjs` 绿 + 无中文标识符）。**G1 三硬用例**（F5 恢复 / 切会话 / abort 不误推）+ browser/肉眼待用户验。commit·**待用户 push**。**下一步 G2 展开态 banner + CTA 调度**。
>
> - **5.178 P1 尺度诚实落地（review.py scale_paradigm_fit desc 强化 + U7 三态）**（v1.0 plan §八 P1 + §配套 A；**未派 subagent**——承重）：① **scale_paradigm_fit desc 强化**（[review.py:50-54](ai_qa/review.py#L50)）——原仅查"颗粒度匹配"（宏观禁落单点/微观禁泛泛），追加**尺度诚实 U7 三态**：问题尺度细于数据支撑（精确到单点/单街/单建筑）时须含「情绪地图看宏观分布与演变、非精确测量」声明 + 给替代趋势（周边/上级单元），禁假装精确值；三态 无声明→fail / 有声明无趋势→warn / 齐全→pass。**机制已就绪**（scale_paradigm_fit 是客观项，fail→pass=false→revise；warn 不 revise——[review.py:172-178](ai_qa/review.py#L172) 后端兜底），desc 写清分级 Flash 即据判。② **docstring 漂移修正**：[review.py:1-8/17](ai_qa/review.py) + [ai_qa/__init__.py:11](ai_qa/__init__.py) "六条"→"七条"（REVIEW_CHECKLIST 早是七条，docstring 落后）。③ **灰度降级**：[DATA/ai_qa/episodes.jsonl](DATA/ai_qa/episodes.jsonl) 63 条全宏观/GIS 操作，**微观精确问题 0 命中** → "≥10 条 fail 率对比"数据不足，待微观数据积累后补（防 fail 率突增>30%）；[emc-test-cases.md](emc-test-cases.md) 用例 9 更新（desc 已落地 ⬜→🔄，灰度待数据 + browser 验证位）。**验证**：`py -m pytest tests/ --ignore=tests/browser` **207 passed 3 skipped**（[eval_template_flash](tests/eval_template_flash.py) 未破——改 review desc 不动 diagnose 路由）。承重零触（diagnose/四态出口/harness 全不动；review 非 diagnose，改 desc 不破 eval）。commit·**待用户 push**。**下一步 P2 引擎 G1-G4**。
>
> - **5.177 EMC 架构优化评估 → Smart Agent/Dumb Tool 开发内核沉淀**（用户提 EMC/AI·Copilot 架构重构三点+附加任务，要求评估+建议+推倒重来判断+归纳通用策略；进 plan 模式自读 [ai-qa-design.md](ai-qa-design.md) 设计圣经 + harness.orchestrate + SKILL_DEFS 现状；**未派 subagent**——承重）：**核心结论 = 不需推倒重来**——用户描述的架构（NL→意图理解·Agent→编排器+DB→Skill/Toolbox→结果输出·Agent→展示）≈ 现状已精确实现（`diagnoseStep` 意图理解 / `harness.orchestrate` 编排器 / `SKILL_DEFS`+`TOOLS` 标准化工具 / `finalStep`+Review 结果输出+审查 / 三态出口承重），经 CB 三轮验证。逐条评估：① 基本架构 **agree 已实现**；② 领域驱动 **partial**——情绪地图四领域共享同一 dumb tool+L2 数据+4×5 矩阵，**领域 Agent 拆分过度工程**（协调成本>隔离收益，违背 Tool dumb 可组合），保持 domain_lens 行业语境注入已够，作为通用可插拔策略抽象进文档；③ 计划-执行 **partial**——现状已是混合版（diagnose 计划 + runTemplatePath plan-once-execute 0-LLM 轮 + while-loop ReAct 兜底），纯 ReAct 低效已被 P1 路径解决；中途打断=真实缺口但 ROI 中低，列远期。**用户最终定调 = 把经验凝练成开发内核词条「Smart Agent, Dumb Tool」**（聪明只在两端·执行做最笨最稳中间件·编排器机械接线）。沉淀：① [CLAUDE.md](../CLAUDE.md) 顶层新节「AI·Copilot 开发内核」（与演示逻辑链/项目设计哲学同级·全量注入）——三角色(Smart Agent/Dumb Tool/Orchestrator)+四铁律(Tool越dumb越好/聪明只在两端/编排器确定性/计划-执行分离)+新功能判据(会推理→Smart·纯执行→Dumb·协调→编排器)；② 新建 [docs/copilot-architecture.md](copilot-architecture.md) 通用抽象纲领（四层骨架+三铁律(三态出口/抗漂移/标准化数据契约)+EMC↔通用术语对照+领域驱动策略+落地模板清单）；③ [ai-qa-design.md](ai-qa-design.md) 双向链接；④ [harness.js](frontend/js/ai_qa/harness.js) orchestrate 注释内核术语显式化（Smart 计划/Dumb 执行/编排器裁定·**逻辑零改**）；⑤ memory `smart-agent-dumb-tool`。**对 CPD 参考**：CPD 引擎 `deriveGuidance`=客户端确定性编排器（铁律3 同源·不进 LLM），天然遵循内核。**验证**：`py -m pytest tests/ --ignore=tests/browser` **207 passed 3 skipped**（harness 仅注释改，eval 未破）。承重零触（diagnose/四态出口/parseAgentStep/review/SKILL_DEFS 逻辑全不动）。commit·**待用户 push**。**下一步** P1 尺度诚实 → P2 引擎 G1-G4（本内核沉淀不阻塞）。
>
> - **5.176 CPD P0 测试铺底落地（plan v1.0 §八 P0 + GUIDANCE 两项增量吸收）**（plan v1.0 定稿后进 P0；先读 K3 [GUIDANCE_E2E-k3.md](catch-ball/GUIDANCE_E2E-k3.md) 端到端前瞻建议打磨 plan → 进 P0；**未派 subagent**——承重）：① **plan 最小打磨**（[cpd-core-plan.md](cpd-core-plan.md) §八 P0 + 顶部注记，**核心 6 决策 + 承重清单不动**，打磨≠CB 迭代）——交叉链接吸收 GUIDANCE 两项 P0 增量：**A1 谓词级测试基建**（§1.1，谓词导出纯函数 + 谓词真值断言，G1 谓词就绪后启用，把死信号/谓词盲区从评审发现变测试发现）+ **组合场景回归**（§4.4，事件×状态×去重咬合，H1 静默冻结教训制度化）。② **catalog 扩充** [emc-test-cases.md](emc-test-cases.md) 4→11 例：地基行为 5-8（默认折叠欢迎卡 / exit-badge 出口徽章 / 高度自适应 / 历史垃圾桶全清）+ 落地 ⬜ 用例 2（domain_lens threading）/ 3（drift 围栏）+ 登记 9（尺度诚实·P1）/ 10（A1 谓词·G1）/ 11（H1 引擎不冻结·G1）+ 组合场景回归元则。③ **6 browser 脚本全绿** [tests/browser/](tests/browser/)（复用 [emc_helpers](tests/browser/lib/emc_helpers.py) + test_compare_regions 模式）：`test_cpd_collapsed_welcome`（无 LLM 最稳）/ `test_exit_badge`（badge 渲染契约·general neutral 软断言）/ `test_emc_height_adapt`（fold56→exp582→refold56）/ `test_history_clear`（2→0）/ `test_domain_lens_threading`（实测 domain_lens=`['urban_planning','urban_renewal']` thread 回传·软断言兜 LLM 方差）/ `test_drift_fence`（无围栏泄漏·exit2=WARN 兜底）。④ **emc_helpers 补**：`emc_session` 上下文管理器（自管 serve.py + chromium + **进程树 taskkill 清理**修 serve.terminate 致 uvicorn 孤儿跨测试级联冲突）+ `read_predicate`/`wait_predicate`（A1 谓词范式·G1）+ `ChatRequestCapture`（/chat 请求体 domain_lens）；open_emc `#lp-upload` 等 visible→**state=attached**（左栏折叠 hidden 但 change 监听已绑）。**验证**：6 脚本 exit 0 + `py -m pytest tests/ --ignore=tests/browser` **207 passed 3 skipped**（[eval_template_flash](tests/eval_template_flash.py) 未破）。承重零触（diagnose/四态出口/harness/curState 全不动；测试纯只读 DOM/网络）。commit d4b6413·**待用户 push**。**下一步 P1 尺度诚实**（review.py desc + 灰度 + U7 三态）→ P2 引擎 G1-G4。
>
> - **5.175 CB-CPD-03 双模型三轮 → plan v1.0 定稿（CB-CPD 专轨收敛）**（CPD 专轨第三轮/收尾轮；主线程反评价 **未派 subagent**——承重）：DeepSeek + K3 三轮验证 [cpd-core-plan.md](cpd-core-plan.md) v0.4（报告 `SCAN_CPDPlan_03-{deepseek,k3}.md`）。DS 综合 A- 建议收尾；**K3 发现 v0.4 新引入 H1 链式缺陷**（general 短路 × `exit!==undefined` 守卫 × 严格 turnId+1 去重 → 引导永久冻结静默失败，已核实 [panel.js:1161/1181](frontend/js/ai_qa/panel.js#L1161) 链）+ M1（hasAnalysis 死信号）+ M2（hasVisibleEmotionLayer 不判情绪性）。反评价 7 条（agree 4 / partial 3 / disagree 0）→ plan v0.4→**v1.0 定稿**：① **H1 修复**——dispatch 守卫 `exit!==undefined`→**`settled`**（覆盖 general 正常完成轮）+ 去重"严格+1"→**单调递增**（免疫跳号）+ `exit??null`；② M1 row 4 `hasAnalysis=true` 升级 `interpret` 分支（dock 产图桥回 EMC）；③ M2 谓词收紧 +判情绪性；④ L1 U8 改 `#param-panel.is-open` 同步谓词。[cb-journal.md](catch-ball/cb-journal.md) `## CB-CPD-03` 四节 + 收敛声明；[review.md](cpd-core-plan-review.md) §七 定稿声明。**CB-CPD 专轨收尾**：三轮双模型闭环（v0.1→v1.0），核心 6 架构决策全自洽（特征向量真值表 / `.aiq-exit-badge` / 小写 exit ∪ null / `settled` 守卫 / 依赖注入 / streaming 第一），演示 C+→B+，承重零触。commit bc5c5ee·**待用户 push**。**v1.0 进 P0/G1**。
>
> - **5.174 CB-CPD-02 双模型二轮验证 → plan v0.4**（CPD 专轨第二轮；主线程反评价 **未派 subagent**——承重）：DeepSeek + K3 二轮验证 [cpd-core-plan.md](cpd-core-plan.md) v0.3（报告 `SCAN_CPDPlan_02-{deepseek,k3}.md`）。**首轮建议全执行**（DS 12/12、K3 15/15），v0.3 升 **B+**（两份一致，v0.2 B-→B+，↑1.5 级）。**两份独立收敛 2 高优（高置信）**：① **init 循环 import**——v0.3 "导出只读 getter" 致 panel.js↔cpd-guide.js 循环 → 改**依赖注入**（panel.js→cpd-guide.js 单向，init 注入 getter，cpd-guide.js 零 import panel.js）；② **S4 动态变量无源**——"主题倾向 X×Y，排序第 N" 客户端无源（正则抠违背用例 2 反模式）→ 降级「{区域名}的归因已就绪」（复用 `_followUps`:455）。**M1 色名脱节**（已 grep 核实 tokens.css:28-29：very-negative #D85A30 深珊瑚橙，**无"深红"**）→ 文案"深红"改"深橙" + 色名从 theme var 派生铁律。M3 优先级文字矛盾（streaming 第一）。反评价 14 条（agree 11 / partial 3 / disagree 0）→ plan v0.3→**v0.4** 11 点修订（§4.2 色名+S4降级+优先级+组合 / §4.3 依赖注入+expectedTurnId / §4.1 hasImport 注释 / §6.4 S3 路径 / §6.5 引导态不持久化 / §九 getter→注入 / U8 is-open）。[cb-journal.md](catch-ball/cb-journal.md) `## CB-CPD-02` 四节。**承重未动**（纯文档）。commit a572ad8·**待用户 push**。**v0.3/v0.4 可进 P0 测试铺底**（两份共识）；待 CB-CPD-03 验证定稿。
>
> - **5.173 CB-CPD-01 双模型首评反评价 → plan v0.3**（CPD 专轨首轮 catch-ball；主线程反评价 **未派 subagent**——承重）：DeepSeek + K3 双模型首评 [cpd-core-plan.md](cpd-core-plan.md) v0.2（均自读项目文件 + 承重实证，报告 `SCAN_CPDPlan_01-{deepseek,k3}.md`）。**反评价 26 条**（agree 20 / partial 6 / disagree 0；**4 承重证据 grep/read 全部核实属实**）：K3 三 P0 spec 错误（① `.aiq-conclusion` 死信号——cpd-state.js:29 查询无 JS 创建者→curState 永不到 S4，改 `.aiq-exit-badge`；② exit 词表大写——harness 实际小写 `result/gap/partial/ask/drift`、general 短路无 exit，读 `_curTrace.exit`、CONCEPT 改判 intent/skipped；③ 映射 key=curState 矛盾——deriveState 任何可见层→S2 致 S0/S1 不可达，改特征向量真值表）；DeepSeek 演示表现力最短板（功能教程非诊断叙事→S3 空间交互优先 + S4 地图定位 CTA 闭合交互环 + 文案叙事化）。→ plan v0.2→**v0.3** 九点修订（§4.1 信号修正 / §4.2 特征向量真值表 / §4.3 turn-ended `{exit,turnId,intent}`+init 恢复 / §七 curState 措辞 / §九 光环 theme-var / §八 分阶段修正 / §配套A 灰度+U7 三态 / §六 6.4 演示链 / U8-U10）。[cb-journal.md](catch-ball/cb-journal.md) `## CB-CPD-01` 四节；[review.md](cpd-core-plan-review.md) prompt 自包含（CB 协议+纪律+本轮轮次+项目语境+必读文件+模型署名）+ SCAN 命名 `-{model}`；[RULES](catch-ball/RULES.md) §2.1 六轴→七轴（加「演示表现力」10%）+ §4.1 多模型命名；[KNOWLEDGE](catch-ball/KNOWLEDGE.md) §2 演示逻辑链北极星语境。**承重未动**（纯文档；review.py/前端/tests 留 P0-P2）。commit c9eeed0·**待用户 push**。**待 CB-CPD-02** 验证修订落地 + 演示升维。
>
...
>
> - **5.162 CPD 折叠胶囊 + 底部 dock + 比例尺 + 抽屉自适应（用户 6 条反馈之 ①②⑤⑥③）**：①② **折叠欢迎胶囊**（commit 246dec2）[ai_qa.css](frontend/css/ai_qa.css)+[panel.js](frontend/js/ai_qa/panel.js)：sparkle 改 **Gemini 1:1 渐变星**（蓝→紫→品红→珊瑚，放大 22px）；去橙框、加强阴影（附地图胶囊）、背后 **conic 彩虹旋转+呼吸**（Gemini 色，`::before` z-index:-1+isolation，纯 CSS 无组件）；文本 **自适应字号** `_fitCollapsedText`（镜像量测 11-14px 塞 2 行，未来 AI 动态文案保完整）。⑤ **Bottom dock**（[map-controls.js](frontend/js/map-controls.js)+[.css](frontend/css/map-controls.css)）：圆角方→**圆形 32px**（同 time-bar/search）、移**底部正中央**（bottom:8/left:50%）、gap 2px、白底胶囊外壳；重排 **cursor/measure/timeline/2D-3D/reset/north/basemap**、去 +/−；时间轴并入 dock（proxy 点隐藏的 `.time-bar`）；reset **保当前 2D/3D 视角**、north **恢复 2D 正北**。⑥ 比例尺底隙 28→**8**（与 dock 一致）+ 右增 **两 20px +/- 小圆钮**（白半透明、同高区底对齐）。③ **抽屉自适应位置**（新设计铁律）：cpd-state.js `positionDrawer()`——抽屉 left 跟随 **EMC 右沿+10**（拖宽→右移不重叠），打开时 + EMC ResizeObserver 调。**④ 待续**：要素按钮 + param-panel 弹层改自适应抽屉（含修 2b 引入的 `#param-panel left:var(--left-w)` 错位回归）。ESM 四绿。push 待用户。
>
