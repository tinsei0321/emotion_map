...
### 🔄 CB-41 双 bug 根因定位 + 台账 + dsh 排查发起（revision-log 5.257·**待 dsh 报告 → 收敛 plan**）

- **B013 新立**（聚合着色）：后端计数无罪（复算=客观表头部逐字吻合）；L0 无极性点层 → grid-warm 色带高值浅金+零点最深红，与「点数越多越深」用户语义相反；toolbox zonal 入口全员 0.5 同色
- **B012 第 3 次复现**（tip 社区错名）：归属点=要素 bbox 中心而非鼠标位置 → **34/174 精确清单**（22 错名+12 无归属·建设→港务/朝阳路→石板/五龙↔梅子溪等）
- 落盘：CB41 发起文档（docs/catch-ball/discuss/·含无障碍纯文字描述+只读复算脚本+8 题排查任务）；buglog 索引重刷 13 条
- 编号账：CB-38 叙述的「B013=range tooltip」从未登记，本号归聚合着色；range tooltip 问题并入 CB-41 §2.4
- **下一步**：dsh 排查报告（discuss/CB41-..._排查报告_dsh组-2026-08-18.md）→ zcode 收敛定稿 plan（严格档）→ 实施
