...
**任务 A：Feature 4 矩阵↔关键词映射改「语义表」（已诊断，待实现）**
- 现状：[`panel.js`](frontend/js/panel.js) `_buildTopicMatrixIndex` 数据驱动扫格 → 因 sim 给 park_plaza/riverside/residential POI 分了 `urban_operation` 域，致 口袋公园/滨江步道→运营×环境 等反直觉关联（点位层面已验：`yichang_L2_T2` 口袋公园 28/38 落运营×环境，滨江步道 50/70 落运营×环境，但语义应规划×环境）。
- 方案：前端建 `TOPIC_MATRIX_MAP`（topic→1-3 个 domain×element 语义块）覆盖数据扫描；矩阵色/数仍走数据，只把**对应关系**改语义。`_applySync` ≤5 全亮/>5 删最浅档（已就绪）。
- **语义映射表（待用户最终确认/调整）**：
  - pos：网红[运营×服务·运营×文化·更新×文化] · 夜经济[运营×事件·规划×事件] · 滨江步道[规划×环境] · 大南门[更新×文化·更新×服务·运营×文化] · 楚超[运营×事件] · 老街新生[更新×文化] · 文化活动[更新×文化·运营×文化·规划×文化] · 加装电梯[更新×设施] · 卷桥河露营[规划×环境] · 绿道成网[规划×环境] · 江南绿肺[规划×环境·更新×环境] · 长江夜游[规划×事件·运营×事件] · 西坝不夜岛[规划×事件·运营×事件]
  - neg：停车难[运营×设施·治理×设施] · 噪音[规划×环境·运营×事件·更新×环境] · 堵车[治理×事件] · 占道停车[治理×设施·治理×环境·更新×环境] · 底商空置冷清[运营×服务] · 红绿灯[治理×设施] · 施工扰民[更新×事件] · 没电梯[更新×设施] · 收费不合理[运营×服务]
  - neu：口袋公园[规划×环境] · 业态[运营×服务·运营×文化] · 社区服务配套[更新×设施·更新×服务] · 物业[更新×服务] · 老街改造[更新×环境] · 盼电梯[更新×设施] · 规划绿地[规划×环境] · 业态调整[运营×服务] · 盼BRT[规划×设施·治理×设施] · 社区营造[更新×文化]
- 待确认：①停车难 运营×设施 vs 治理×设施（现列两个）；②噪音/占道停车 跨 4 块要不要收窄（>5 会被砍最浅档）；③任何词归错块。
- 涉及：`_buildTopicMatrixIndex`/`_syncFromMatrix`/`_syncFromKeyword`/`_applyStickySync` 改查表；TOPIC_TABLE 在 [`performance_config.py`](SCRIPT/performance_config.py)。
- 备选方案：改 sim 数据（park_plaza/riverside→规划域，重跑全量）——动静大、矩阵分布会变，不推荐。

**任务 B：单元深读 12 词典型格定稿**
- 候选池见 [`revision-log.md`](docs/revision-log.md) 5.16「单元深读靶心表」+ 5.18 点军 3 词补充。12 词：网红/夜经济/大南门/楚超/卷桥河露营/江南绿肺（pos）· 停车难/占道停车/堵车（neg）· 口袋公园/业态/社区服务配套（neu）。
- 每词从候选聚合域筛 1-3 典型格（按 point_count top + topic_top==词 命中），跑数据扫描给 cell ID 定稿。plan `C:\Users\Hi\.claude\plans\revision-log-5-19-misty-map.md` Phase 5 表有推荐聚合域。
