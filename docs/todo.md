...
- [ ] **定稿发 zcode**：落 `03_元数据/分类对标映射表.csv`（两轨桥梁唯一权威）+ 体检侧加列（77 项 3 列/问题清单/05 链条/fact 卡·附件2 低优先）
- [ ] **claude 组 12345 侧重跑**：治理脚本加 4 列（task_8/project_type/board/小类映射）→ `12345_分类映射.json`（42 中类→12 类型 + 213 小类跨界手工 <20）→ 重跑重跑验证两板块占比
- [ ] 三组复核（diff 原名不变·无 None·占比突出）
- [ ] 出图数表讨论（重构后）→ 3' 开动
