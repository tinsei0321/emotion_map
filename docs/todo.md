...
### ⏸️ RAG 建设（本周重点·记入计划·知识库完成后承接）

- **背景**：用户问"宜昌有哪些更新项目"→ 暴露 EMC 只注入不检索·需 RAG（向量化 L0/outlet_kb）
- **方案已落盘**：`docs/catch-ball/discuss/EMC-RAG建设方案_本周重点_2026-08-09.md`
- **承接**：知识库 + L2 完成后 → RAG 专题讨论（推翻 CB-14 前提·现状已变：知识库暴涨/数据检索需求）
- **待办**：验证 embedding 能力（火山 multimodal/智谱）→ 建向量库 + rag_search 工具 → 接 diagnose 意图
