...
### 决策 3：软折叠基调不变（非严格隐身）+ 双域架构（v1.2）
chip/控件始终可达，引导 = 高亮 + 文案 + 光环 + CTA，不强制隐身其他控件。engage 解除（光环停而非控件锁死）。

**双域架构（v1.2·用户重新定义）**：EMC 与 CPD 是**独立两个域，互不干扰**——
- **收起态**：CPD 接替 EMC（光环引导胶囊提示下一步）；EMC 隐藏。
- **展开态**：EMC 接手（对话/agent 做详细解释与操作）；CPD **同步进 EMC 界面**作**缩小提示条**（`.emc-cpd-hint`·进度点上方·去光环·阴影加深·Light/Dark）继续提示。两者并列。
- CPD = 引导域（确定性·提示"下一步做什么"）；EMC = 解释/操作域（LLM agent·思考/解析/结论）。展开后用户首次输入才触发 EMC harness（不展开就耗 LLM）。
...
|---|---|---|---|---|---|---|---|
| **false** | * | * | * | false | `null` | **v1.2（空态欢迎语·用户定）**：空态显「向 Copilot 提问：了解情绪地图…」欢迎语，**不一上来引导操作**；点开折叠胶囊→欢迎卡。~~import 引导~~ 删除（用户自主用左栏导入，导入后才入装载链 row 5+） | — |
