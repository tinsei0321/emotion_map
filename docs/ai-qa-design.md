...
## 第 5.5 章 · 回答策略 = 三态出口契约（执行纲领，2026-07-10）

> 替代旧「第 4 章 SOP」的**执行层**描述。第 4 章 SOP 的域知识（先判 intent / 先 query 后操作 / 数据驱动 / 指向城建问题）仍有效，但**何时算"答完"**由本契约裁定——且由 **harness 代码强制，非靠模型自觉**。
> 出处：CARTO《构建地理 AI Agent 的教训》"多数 agent 失败是结构失败，不是模型失败""把每步显式呈现建信任"；QGIS Copilot(Akinboyewa 2025)"高级任务难自主→优雅交接不可或缺"。

### 起因（根治"只说不做"+代码块泄漏）

反复毛病：EMC 把工具调用以 ```json 代码块糊进答案、讲计划不执行。根因（读码确认）：
1. **出口逻辑只在 prompt**（manifesto §5/§8/§10 + FINAL 诚实铁律都对），全靠模型自觉；
2. **格式漂移→裸输崩塌**：DeepSeek 输出非约定 schema（`{action:"x",arguments:{}}` / prose 包裹 ```json```）→ `parseAgentStep` 返畸形/失败 → harness 8 轮空转 → `panel.onDegraded` 把**原始 token 糊进答案泡**（=用户看到的代码块）；
3. **hotspot 不落图层**（密度/热点类不出图）；**入参别名漂移即失败**（inverse/output_layer/radius）。

### 三态出口契约

每次问答**必**落到且仅落到三种干净终态之一，由 `harness.js orchestrate` 裁定：

| 终态 | 触发（代码裁定） | 出口形态 | 绝不允许 |
|---|---|---|---|
| **EXIT_RESULT 做成了** | intent∈{B,C} 且 `successObs>0` 或 `newLayerCount>0` | 数据驱动结论 + `{{focus/show/inspect}}` 操作按钮；B 类必带"已生成图层X" | 计划文、代码块、空话 |
| **EXIT_GAP 做不成** | `strategy=request_upload`，**或** intent∈{B,C} 且 `successObs===0 && newLayerCount===0` | 确定性"缺数据卡"（缺什么/为何/已尝试/引导上传或换问法），`composeGapCard` 组装**不走 LLM** | 编造、假装做成、口头计划 |
| **EXIT_CONCEPT 纯问答** | intent=A 通用 | 直接简洁答 | 硬塞 4×5/演示链 |

**最高杠杆单点**：intent∈{B,C} 零成功时**禁止**走叙述型 `finalStep`，直接 EXIT_GAP——杀死"只说不做"。

### 落地（代码层强制）

- **抗格式漂移**（`stages.js parseAgentStep`）：归一 drift schema（action 字符串 / `{tool,params}` / fenced）+ 入参别名规整（解析期单源）+ 纯叙述返 `{narrated:true}` 哨兵。**格式漂移注入测试 8/8 通过**。
- **叙述修复通道**（`harness.js`）：`narrated` 时纠偏重发 ≤1 轮，仍不成 → EXIT_GAP；**永不裸输原始 token**。
- **永不裸输**（`panel.js onDegraded`）：固定降级卡，忽略传入 raw 文本（代码块泄漏的最终保险）。
- **终态徽章**（`panel.js _exitBadge`）：答案尾显「已生成 N 个图层 / 缺数据·需上传 / 纯问答 / 分析完成」。
- **密度出口**：新增后端 `kde_raster`(F_005) + `/geo/density` + 前端 `density` 工具（离散分段色带，2D 复用 isTool 色带 fill 管线）；`hotspot` 修落图层（hot=红/cold=绿/ns=灰）。

### 文件映射

| 关注点 | 文件 |
|---|---|
| 终态机 + 守卫 + composeGapCard | `harness.js` |
| 抗漂移解析 + 别名 + narrated | `stages.js:parseAgentStep` |
| 永不裸输 + 终态徽章 | `panel.js` |
| density 工具 / hotspot 落图 | `tools.js` |
| KDE 后端 | `core/spatial_analysis.py:kde_raster` + `api/geo_routes.py:/geo/density` |
| 出口契约入 prompt | `prompts.py` + `manifesto.py §8` |
