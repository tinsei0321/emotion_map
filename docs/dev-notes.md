...
### 2026-08-18 | CB-41 体检点聚合双 bug 三轮修复——踩坑蒸馏为全局调试记忆

**关键词**：CB-41, B012, B013, B014, membership 静默丢点, 异构属性 GeoJSON, 多入口验证, debug-memory

#### 做了什么
- 用户报双 bug（聚合着色反语义 + tip 社区归属错乱），走 CB 全流程：发起（zcode 根因复算 + 34/174 精确清单）→ dsh 独立排查 → 收敛定稿 → 实施（`126537eb`）→ 用户复测暴露第三层 B014（`/spatial/aggregate` membership 值匹配静默丢点 1696/2296·中央社区计数清零）→ 增补修复（`38215ab3`）。
- 应用户要求把全程踩坑蒸馏为 **`docs/debug-memory.md`**（R1-R10 规则库·多组共享），AGENTS.md / KNOWLEDGE.md / context-map 三处挂载普及。

#### 遇到的问题
- 排查期只验证了函数直调 + `/geo/zonal_stats`，漏了 grid「指定单元」的 `/spatial/aggregate` 路由（参数默认值与数据构建方式不同）→ 「已验证」交付后用户复测翻车 → **R1 多入口逐入口验证 / R8 验证忌同构**。
- 异构属性点文件（600/2296 带社区列）from_features 联合后 74% NaN，被 `isin` 整行静默丢弃 → **R2 空值显式处理·禁静默丢数据**。
- 零点透明修复让计数为零第一次以「无填充」显影，症状迁移差点误导回改渲染 → **R3 数据→映射→渲染三层判别**。
- 修复提交后用户后端仍是旧进程 → **R7 交付口径必含重启**。

#### 收获 / 心得
- 同族 bug 反复发作的根因是「修症状层没修语义层」——入口 × 错误语义是乘积关系，语义层修对各入口自然收敛（**R10**）。
- trace.log（F_003 时长冷/热）+ netstat（端口 PID）能在纸面推演穷尽后一步还原用户操作轨迹与进程拓扑（**R6**）。

### 2026-06-27 | 产品架构升级 — 单页到三页（数据库→控制台→实时地图）

**关键词**：三页架构, 职责分层, 人民城市, 数据驱动, ADR-015, L0~L4 双视角

#### 做了什么
- 产品顶层从"单一地图应用"重构为**三页架构**，自下而上：数据库（emotion-database）→ 控制台（emotion-console）→ 实时地图（emotion-map）。决策落档 ADR-015；同步更新 `architecture.md`（§2 三页顶层图 + §2.2 七层=控制台内部 / §4 L0~L4 双视角 / §8 三页演进）、`prd.md`（§1.5 三页 + §3.1 三页功能总览）。
- 明确三页各自定位与面向角色：① 数据库=运维侧数据汇总库（L0 采集/购买→治理→L1，含 L0~L4 各类/时段/语料/临时数据）；② 控制台=规划研究工具（当前 α v0.1 焦点，本仓库 `frontend/`）；③ 实时地图=商用实时服务（控制台内核+动态数据+动画/UI+AI+移动端）。

#### 思考 / 心得
- **为什么分三页**：三类角色（运维/研究/商用）场景差异大，单页会让商用/运维功能污染研究工具。职责分层 = 各页专注一类角色、可独立立项扩团队。
- **数据流是主轴**：数据库供数 → 控制台分析 → 实时地图服务。控制台内核是实时地图的基础，复用设计要前置。
- **L0~L4 双视角澄清**：过程视角=分析管道（L0→L1→L2→L3→L4），数据视角=各阶段产物作为数据库的数据类型分层。同一套 L0~L4 既是过程也是数据分类，不矛盾。
- **"人民城市"落地**：跳出传统 GIS 平台局限，用"数据驱动"解决"市民需求"——三页分别服务三类城市工作角色，是这理念的工程化分解。
- **当前聚焦**：控制台 α v0.1 验收 → 立项 → 扩团队开发数据库与实时地图，资源不分散。
