...
### 2.1 产品三页架构（顶层 · 2026-06-27）

情绪地图产品由**三个网页**组成，整体架构**自下而上**：数据从底层数据库流入，经控制台加工分析，最终在实时地图对外服务。

```
+-------------------------------------------------------------+
|  网页(3)  实时地图 emotion-map           [待开发 · 商用]     |
|  面向：城市运营 / 治理部门与企业（商用客户）                 |
|  基于控制台内核的实时展示 / 筛选 / 分析 + 动画 UI -> 实时洞察|
|  · 热点 / 热力 / 网格 · 总量趋势 / 极性类型表现              |
|  · 历史数据(日/周/月/季/年) + 实时情绪云图 · AI 赋能 · 移动端|
+-------------------------------------------------------------+
|  网页(2)  控制台 emotion-console         [开发中 · α v0.1]   |
|  ←←← 当前焦点（本仓库 frontend/） >>>                        |
|  面向：规划设计 / 研究人员                                    |
|  支撑规划方案 / 课题研究、生产分析报告的可视化地图工具        |
|  · 数据上传 / 展示 · 情绪快照(总体 / 类型细分 / 时间轴)      |
|  · 快速分析(Overview+Table) + 专业分析报告                  |
|  内核 = 本仓库 frontend/（MapLibre GL JS）                   |
+-------------------------------------------------------------+
|  网页(1)  数据库 emotion-database        [待开发 · 内部运维] |
|  面向：开发人员 / 运维                                       |
|  各类型 / 周期性 / 临时性数据汇总库，为控制台与实时地图供数  |
|  · L0 采集 / 购买 -> 数据治理 -> L1 城市情绪数据             |
|  · L0~L4 各类数据 + 不同时段 / 周期性 + 语料库 + 临时数据    |
|  -> 结构化、系统化的"情绪地图数据库"                         |
+-------------------------------------------------------------+
```

> **当前焦点 = 控制台 α v0.1**（本仓库 `frontend/`）。数据库与实时地图待控制台测试通过、顺利立项后扩大团队开发。三页架构决策详见 ADR-015；控制台 α v0.1 已实现功能详见 [PRD §3](prd.md)。
>
> **意图**：在"人民城市"大背景与城市更新 / 运营新指导思想下，跳出传统 GIS 平台局限，用"数据驱动"解决"市民需求"——三页分别服务运维、规划研究、商用运营三类角色。

### 2.2 控制台内部七层架构

> 下方七层架构描述的是**控制台页（网页②）的内部实现**。架构按**从下至上**的顺序组织：数据从底层流入，逐层加工，最终在应用层呈现。
>
...
> **L0~L4 双视角（2026-06-27 澄清）**：
> - **过程视角 = 分析管道**：L0(原始) → L1(治理) → L2(SnowNLP 情绪) → L3(LLM 语义) → L4(归因)，逐级叠加，每级不破坏前级字段。
> - **数据视角 = 数据库数据类型**：管道各阶段产物（L0 原始 / L1 治理后 / L2 情绪 / L3 LLM 增强 / L4 归因）各自构成"情绪地图数据库"（网页①）中的一类入库数据，可独立存储、检索、复用。
>
> 即：L0→L4 既是分析过程，其产物又是数据库的数据类型分层。L0/L1/L2 已明确，L3/L4 接口预留。
...
> **产品架构演进（三页架构 · 2026-06-27）**：产品按"数据库 → 控制台 → 实时地图"三页推进。
> - **当前阶段 = 控制台（网页②）α v0.1**：数据上传 / 展示 + 情绪快照 + 分析报告，内核为本仓库 `frontend/`。
> - **下一阶段 = 数据库（网页①）**：L0 采集 / 购买 → 数据治理 → L1 城市情绪数据，构建结构化情绪地图数据库。
> - **远期 = 实时地图（网页③）**：基于控制台内核的商用实时地图服务（实时云图 / 动画 / AI / 移动端）。
>
> 下方 Phase 1~3 为**控制台页内部**的技术演进路线。
