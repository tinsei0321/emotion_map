...
│                     🕷 数据采集层 (SCRAPER/)                      │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  data_scraper.py        多源数据爬取引擎（待建）            │   │
│  │  · 大众点评 / 美团 / 小红书 / 微博 / 12345 投诉            │   │
...
> 状态：方案调研中，预计 2026-06-13 前明确技术路径

| 数据源 | 内容类型 | 获取方式 | 优先级 |
|--------|----------|----------|--------|
| 大众点评 | POI 评价文本 + 坐标 | 爬取 / 购买 | ⭐⭐⭐ |
| 美团 | 商户评论 | 爬取 / 购买 | ⭐⭐ |
| 小红书 | 笔记/攻略（城市体验类） | 爬取 | ⭐⭐ |
| 微博 | 签到/话题（事件感知） | API / 爬取 | ⭐ |
| 12345 热线 | 投诉工单（带地址） | 政府合作 | ⭐⭐⭐ |

**技术选型（拟定）**：
- **Scrapy**：开源爬虫框架，成熟稳定，插件丰富（代理/IP池/去重），适合多数据源统一管理
- **备用方案**：购买数据（保证稳定批量获取，未来产品化不可能依赖手动爬取）

**输出规范**：所有爬取数据统一输出到 `data/raw/`，文件命名 `{source}_{date}_{area}_raw.csv`
