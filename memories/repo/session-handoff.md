...
- **软指标可信性缺口**（立项切入点）：城市体检 61 项基础指标·客观 ~79%（遥感/CIM/普查·不含民意）+ 主观感知 ~21%（问卷·样本≥0.2%·时滞/难定位/难追踪）——缺"既承载民意又统计可靠可定位"的软指标层 = 情绪地图要填的（全量评论流）
- **地方特色指标**：官方"基础+特色"两类框架·特色=基础之上增加（非第三类并列）·特色里主观类占比更高
- **outlet_kb**（`ai_qa/outlet_kb/`）：7 契约（更新对象识别/需求摸排/时序/内容/项目 + 体检满意度/四维度）+ 8 指标 + 5 案例（宜昌望洲岗/上海/广州/南京/宁夏）·确定性组装（build_outlet_schema 查契约·不靠 LLM 编造）·能/不能双栏
