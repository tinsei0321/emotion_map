...
> 最后更新：08月12日（office 环境·紧急任务城市体检两板块·**12345+体检统一分类对标重构·三组讨论回收·收敛定稿已落盘·待执行**）| 分支 `main`
>
...
## 当前节点：分类对标重构（三组讨论回收 → 收敛定稿 → 待执行）+ 出图数表讨论

08-11/12 大轮（跨项目工程·城市体检两板块·双项目交叉）·定稿五阶段推进·**当前前置步骤 = 12345+体检统一分类对标重构**（用户明确·逻辑闭环：调研/收集→标准化治理→对标体检指标门类→体检问题）：
...
| **分类对标重构** | 12345（8 大类/42 中类/213 小类）+ 体检（77 项/16 子维度）统一分类体系 | 🔄 **三组讨论回收·收敛定稿已落盘·待执行** |
| 3' 两板块分析 | 图/数/表/观点 | ⬜ 重构后 |
...
## 【下一步】（分类对标重构执行 → 出图数表讨论 → 3' 分析）

1. **分类对标重构执行**（收敛定稿 `docs/catch-ball/discuss/CB23-12345分类对标重构_收敛定稿_2026-08-12.md` 为唯一执行依据）：
   - ① 定稿发 zcode → 落 `03_元数据/分类对标映射表.csv`（两轨桥梁唯一权威）+ 体检侧加列（77 项加 3 列不改名/问题清单/05 链条/fact 卡·附件2 低优先）
   - ② **claude 组**：治理脚本加 4 列（task_8/project_type/board/小类映射）→ `12345_分类映射.json`（42 中类→12 类型 + 213 小类跨界手工 <20）→ 重跑验证两板块占比（民生预期 >60%·安全韧性 ~15-20%·尊重事实）
   - ③ 三组复核（diff 原名不变·无 None·占比统计）
2. **出图数表讨论**（重构后·当前唯一门槛）：图1-4 呈现链·2×3 矩阵·分析内容×方法×结果形式·Codex 方案 A
3. **3' 两板块分析**（不卡边界·问题→项目趋势聚拢→三片区统筹）
...
- 当前：**CB-23 分类对标重构**（发起 → Codex+zcode 回收 → 收敛定稿 ✅ 闭环 → **待执行**：zcode 落映射表 + claude 重跑治理脚本 → 三组复核）
- 定稿关键：12 项目类型（Codex 案）·矛盾拆+残留归其他·智慧高效按项拆分（zcode 案）·子维度 16 更正·执行顺序=映射表→加列→重跑→diff 校验
