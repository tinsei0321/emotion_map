...
metadata: 
  ...
  modified: 2026-08-15T15:12:43.532Z
---
...
**How to apply:**
- **纪律（一键两入口·2026-08-15 晚起）**：下班双击 HUB 根 `一键离开.bat`（=Leave+Memory+蒸馏层 push+origin 兜底）；上班双击 `一键到达.bat`（=Arrive 快进+Gitee pull+`memory_sync.py restore` 合并式恢复·只补新不删）。原 Leave/Arrive/Memory/Status/Rescue 单入口保留可单用；跨机衔接 = 会话开场读 `E:\DEV-SYNC-HUB\memory\<对端>\claude\` 最近会话尾部 + 对端 MEMORY.md；`Memory.bat pull`/`memory_sync.py pull` 仅迁移/灾难用（/MIR 覆盖·日常禁跑，restore 才是日常）
