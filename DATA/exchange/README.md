...
| 04 互通优化 | `{URENEWAL_ROOT}/…/04_互通优化/` | EMC 桥接（RAG 素材 fact卡/摘要/图表JSON/图层元数据 + 双轨对比表 + 面化方案 + 双轨数据地图）·阶段 2 消费 |
| 06 主观数据治理 | `{URENEWAL_ROOT}/…/06_主观数据治理/` | 主观数据治理产物（12345 治理清洗版/情绪地图中转版/4x5 映射 + 迁移 情绪_12345片区面/问卷片区指标/双轨对照索引/图15任务单）·CB-23 zcode 查源 v2 迁移（04→06） |
