...
### 结构性缺失（情绪地图的立项切入点）
城市体检指标体系存在**"软指标可信性缺口"（soft-indicator credibility gap）**：
- **硬指标太硬**（~79%·遥感/CIM/普查·客观可靠·但数据本身不含市民声音）
- **主观/感知指标太飘**（~21%·居民问卷抽样调查·含民意·但采集不可靠：样本≥0.2% 人口/年度时滞/难定位/难追踪验收）
- **缺口** = 缺一个"**既承载市民民意、又统计可靠、可空间定位**"的软指标层——**这正是情绪地图要填补的**（全量评论持续流·更客观/更有说服力/更科学）。
- 官方表述：住建部《关于全面开展城市体检工作的指导意见》（建科〔2023〕75号）·四维度（住房/小区/街区/城区）61 项基础指标 + 地方"增加"特色指标（基础之上增量·非第三类并列）。
