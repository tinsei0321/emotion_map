...
把"除草"从手动反复操作沉淀成统一流程：判等级 + 列清单 + 批准 + 留痕。复用 `/sync-log`（不重复同步逻辑）。退役台账 [docs/catch-ball/retired.md](docs/catch-ball/retired.md)。
...
5. **留痕**：追加 [docs/catch-ball/retired.md](docs/catch-ball/retired.md)（退役台账：日期 + 清单 + 理由）。不存在则创建（标题 + 表头）。
