...
bug 标记 `status: resolved` + 补「修复记录」后，跑 `py tests/buglog/_gen_index.py` → 自动重生 `tests/buglog/_regression.md`（从条目 body 提取问句 + 预期 + 根因指针）。

**定位（诚实）**：`_regression.md` = 发版前**手动复验清单**，非飞轮自动执行。原因：
- 数据前提逐案不同（如 B001 需「带字段 `MC` 的面层」，而飞轮 `llmRun` 数据 setup 只支持点层/范围）→ 无法从语义描述自动装配可执行用例。
- 语义「预期行为」（如「结论诚实」）无法可靠转成断言，强行自动执行会**虚假绿**（比无测试更危险）。
- 关联飞轮用例（`case_ref`，如 TC-06）在常规飞轮跑里**已覆盖执行**——回归保护由它承担，buglog 清单补「别忘手动复验」。

仪表盘「回归关注」节列已修复 bug + `case_ref`，发版前照单复验。
