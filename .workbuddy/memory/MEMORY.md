...
- 学习报告必讨论必落盘，无回应不收敛。
- **派发 prompt 义务（2026-08-25 用户令）**：凡涉及「给 xx 看/审计/讨论」等有对象的转交动作，必须同时产出一份**一键复制即用的派发 prompt**（含背景、对象、任务、材料路径、验收要求）。**交付形态：prompt 必须落盘为文件（`docs/catch-ball/discuss/_prompts/` 目录）并 present 给用户**——仅放聊天代码块不算完成（用户反馈无复制按钮·2026-08-25）。
