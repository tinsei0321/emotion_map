...
import { applyQualityDefense, _setLastToolRowsForTest, _buildOutletCardForTest, composeGapCard, _quickIntent, _assembleKnowledgeQA, _composeDegradedConclusion, _followupCue, _buildExtracted } from './ai_qa/harness.js';   // G0 R9 单测 + Wave 1 出口卡直测 + ③w5 措辞断言 + CB-22 RAG 短路判定 + P0-5 合流直测 + CB-22e P2 降级结论直测 + CB-22f 衔接分类器直测
...
window.__emcTest = {
  ...
  composeDegraded(t) { return _composeDegradedConclusion(t); },   // CB-22e P2：降级结论直测——观察行优先规则（命中 N/M 取首·修部分命中断链）
  followupCue(ctx) { return _followupCue(ctx); },   // CB-22f D4：衔接分类器直测（双条件守卫·8 场景桩测）
  buildExtracted(r) { return _buildExtracted(r); },   // CB-22f D3：识别层组装直测（fact 字段→geo/attrs·确定性）
