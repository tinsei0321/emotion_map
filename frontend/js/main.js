...
// ── E2E test seam（?e2e=1）──────────────────────────────────────────────────
// 地图空启动（下方 main() 内 "No seed sample"），browser E2E 测试需注入 fixture 点层
// 供 zonal_stats/compare 聚合。仅 URL 含 e2e=1 时暴露 window.__emcTest.loadPoints(fc)，
// 复用 Import 的点层装载逻辑（splitByGeometry + detectColorMode + L2 极性拆分 + addLayer/renderLayer）。
// 零生产影响：普通访问无 flag → 不暴露，不执行。tests/browser/ 用此注入 fixture。
if (typeof window !== 'undefined' && new URLSearchParams(location.search).get('e2e') === '1') {
  window.__emcTest = {
  ...
  console.log('[e2e] window.__emcTest.loadPoints ready');
}
