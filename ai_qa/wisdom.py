...
WISDOM = [
    ...
    {
        "id": "culture-block-attribution",
        "scale": "meso",
        "domains": ["urban_renewal", "urban_operation"],
        "scenario": "历史街区/文化街区深归因（“二马路/老街焕新好不好、烟火气保留没”类）",
        "do": "zonal_stats(街区 boundary) + 4×5 深读(更新×文化/更新×环境) + 政策→项目闭环（防止大拆大建/留改拆 → 具体保护更新项目）；aspect 级（修旧如旧/烟火气/业态）非整体极性",
        "dont": "泛泛“老街有韵味/有文化气息”——文化街区要落到具体活化项目 + 烟火气/记忆保留评估",
        "exemplar": "“二马路历史街区 更新×文化：修旧如旧获好评（打卡墙/花墙出片），但业态同质化 + 烟火气商业化流失担忧显现；政策锚=防止大拆大建（不大规模拆除/搬迁）→ 项目=历史街区保护更新，建议业态多元化 + 原住民保留”",
    },
    {
        "id": "planning-meso-locate",
        "scale": "meso",
        "domains": ["urban_planning"],
        "scenario": "街区/片区级规划落点（“这片该补什么设施/绿地怎么布”类）",
        "do": "zonal_stats(街道/单元) + overlay(用地层) → 缺口识别（设施/绿地短板落空间）+ 规划项目建议（参照 15 分钟生活圈/完整社区）",
        "dont": "落单点 / 只报情绪不接规划项目——中观规划要“设施短板 + 用地匹配 + 项目建议”",
        "exemplar": "“XX 街区 规划×设施：情绪反映配套不足（养老托幼/公共空间），overlay 显示居住用地多但公共设施用地缺；建议按完整社区标准补齐社区中心 + 口袋公园（15 分钟生活圈）”",
    },
    {
        "id": "operation-event-transient",
        "scale": "meso",
        "domains": ["urban_operation"],
        "scenario": "赛事/演唱会/节庆瞬时影响（“奥体办活动周边怎么样/五一爆满”类）",
        "do": "hotspot/hex(事件时段) + filter_attr(element=事件) → 瞬时人流聚集的空间影响（拥堵/秩序/周边商业）；这是官方体检按日均评估忽视的盲区，EMC 差异化价值",
        "dont": "只报活动好坏不落空间 / 用常态日均代替瞬时峰值",
        "exemplar": "“奥体中心演唱会期间 运营×事件：周边 Gi* 显著热点（拥堵/噪音），瞬时人流拉爆停车+秩序；建议临时交通管制 + 周边商业引流。官方体检按日均值评会遮蔽瞬时峰值——EMC 补这个盲区”",
    },
]
