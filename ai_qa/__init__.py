...
三层 Harness 架构（详见 docs/ai-qa-design.md）：
- 知识层 manifesto.py    —— MANIFESTO 领域宪法（喂给模型，让它真懂情绪地图）
- 思考层 prompts.py      —— build_agent/diagnose/final/optimize_prompt（SOP 解题协议）
- 执行层 （前端 tools.js，tool 操作地图；后端无）
（CB-09 D022：审查层 review.py 已删 → 前端 harness.applyQualityDefense 代码防线取代 LLM 审查+重写）

入口：router.py 暴露 /chat（phase: diagnose|agent_step|answer|optimize），挂载到 api/main.py。
