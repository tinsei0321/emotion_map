...
def build_fc_sys_prompt(context):
    ...
    return (
        ...
        '## 知识问答选型（CB-22f A·双条件守卫·防误判 GIS→knowledge）\n'
        '- 用户问题属**纯问答**（问"有哪些项目/是什么/多少/政策/指标/体检/做法"·无空间操作意图）→ 选 **knowledge_qa**（检索本地知识库文字回答·不产图层）\n'
        '- **双条件守卫**：仅当①知识问词命中（项目/指标/体检/政策/案例/问题/做法/机制/有哪些/是什么/多少）**且**②无空间操作意图（不命中 核密度/热力/裁剪/缓冲/叠置/排序/聚合/网格/分布/情绪/极性/标记/地图 类词）→ 才选 knowledge_qa\n'
        '- 含空间操作/情绪分析意图（哪怕带地名）→ 仍选 GIS 工具（knowledge_qa 不适用·如"西陵区情绪分布"是 emotion_analysis 非 knowledge_qa）\n'
        '- 纯问答意图无法用工具表达时 → 选 knowledge_qa（不要硬选 GIS 工具）\n'
        f'## 数据上下文\n{context or "（无数据上下文）"}\n'
    )
