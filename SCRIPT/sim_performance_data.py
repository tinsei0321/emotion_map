...
from performance_config import SNAPSHOTS, pick_polarity, pick_domain_element, snapshot_flavor, AREA_TYPES
...
def load_boundaries():
    ...
    safe_print('[LOAD] cc/core 边界就绪（unit=锚点 buffer，无需边界文件）')
    return cc, core
...
def _seed_domain_element(snapshot_id, area_type, poi, rng):
    """层1(80% 有 POI)：继承 POI 的 domain/element；层2：performance_config 区域 bias。"""
    if poi and rng.random() < POI_INHERIT_P:
        d, e = poi.get('domain'), poi.get('element')
        if d and e:
            return d, e
    return pick_domain_element(snapshot_id, area_type, rng)


def _pick_polarity_clustered(snapshot_id, area_type, poi, rng):
    """区域 arc + POI lean 翻转（局部纹理）。"""
    base = pick_polarity(snapshot_id, area_type, rng)
...
def inject_fields(pts, snapshot_id, pool, pl, poigrid, rng):
    ...
    for i, p in enumerate(pts):
        ...
        if in_cc:
            ...
            polarity = _pick_polarity_clustered(snapshot_id, at, poi, rng)
            domain, element = _seed_domain_element(snapshot_id, at, poi, rng)
            ...
            rows.append({
                'lon': p['lng'], 'lat': p['lat'], 'area_tag': at, 'zone': zone,
