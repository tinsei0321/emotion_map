...
溯佰科平台说明：
  溯佰科是城市规划时空大模型平台（集成城市数据底座、GIS工具、
  自然语言工作台）。情绪地图未来以 Agent 方式嵌入溯佰科，
  通过 LLMAnalyzer 接口调用其自然语言处理能力完成 L3 语义增强。
...
class LLMAnalyzer(AnalyzerBase):
    """
    【L3】LLM 语义增强引擎（溯佰科城市规划时空大模型平台）。

    溯佰科是城市规划时空大模型平台（集成城市数据底座、GIS工具、
    自然语言工作台），情绪地图未来以 Agent 方式嵌入溯佰科，
    通过本 Analyzer 接口调用其自然语言处理能力。
