...
# Codex P2-3：`请(及时|尽快|马上)处理` 整词匹配（防「处理」残留·原 `请及时处理` 部分删除留「处理」）
_FLOW_RE = re.compile(r'（.*?）|于\d{1,2}[:：]?\d{2}.*?转接|转接.*?工作人员|交办至.*?处理|热线\d+'
                      r'|受理|工作人员|三方通话|12345|请(及时|尽快|马上)?处理|请帮忙|请马上|请尽快|请及时|麻烦处理|投诉热线|反映人|来电人')
_SPEECH_RE = re.compile(r'我是[^，。；]{0,20}?居民|我是[^，。；]{0,20}?业主|我住在[^，。；]{0,10}|居住在[^，。；]{0,10}'
                        r'|本人|我们这|我们这栋|我于\d{1,2}月\d{1,2}日')
# Codex P2-3：`(早上|上午|中午|下午|晚上|凌晨)?\d{1,2}点(半|多)?` 整段匹配（防「多」残留·原 `晚上11点多` 删「晚上11点」留「多」）
_TIME_RESID = re.compile(r'(?:早上|上午|中午|下午|晚上|凌晨)?\d{1,2}点(?:半|多)?|(?:早上|上午|中午|下午|晚上|凌晨)\d{1,2}[:：]\d{2}|\d{1,2}[:：]\d{2}')
